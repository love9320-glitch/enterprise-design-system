...
import { SelectChip } from '../components/Select';
...
import type { ScreeningConditionValue } from './ScreeningConditionCard';
import { ScreeningIndividualSettingModal } from './ScreeningIndividualSettingModal';
import { CHIP_COLOR_CLASS } from '../components/chipStyles';
import { FORMULA_FN_FAMILY, FORMULA_GROUP_FUNCTIONS, focusNextChainStop, chainTriggerKey, hasPointsScore } from '../components/formulaFunctions';
import type {
  FormulaCatalog,
  FormulaGroupNode,
  FormulaLeafNode,
  FormulaNode,
  OptionItem,
} from '../components/formulaFunctions';

// 표기 변형 — 'formula'(수식 표기) | 'natural'(자연어 문장 표기)
type FormulaVariant = 'formula' | 'natural';

// 계열별 정적 클래스 매핑 (Tailwind purge 안전)
// 함수 계열 → 칩 color(2026-07-15 — 자연어 그룹 함수 셀렉트 칩 채색)
const FAMILY_CHIP_COLOR: Record<string, 'green' | 'blue' | 'violet' | 'pink' | 'orange'> = {
  logical: 'green',
  conditional: 'blue',
  aggregate: 'violet',
  'score-limit': 'pink',
...
const FAMILY_HOVER_BORDER: Record<string, string> = {
...
function FormulaShell({
  ...
  family: string; // 함수 계열(hover 아웃라인 색)
  dense?: boolean;
  comment?: ReactNode;
  className?: string;
  children?: ReactNode;
  ...
  useEffect(() => {
    ...
    const over = (e: MouseEvent) =>
      setHovered((e.target as Element | null)?.closest('[data-formula-shell]') === el);
    const out = (e: MouseEvent) => {
      if (!el.contains(e.relatedTarget as Node | null)) setHovered(false);
    };
...
const Comparison = ({ children }: { children?: ReactNode }) => (
...
const NaturalText = ({ children }: { children?: ReactNode }) => (
  <span className="shrink-0 text-12 text-formula-comparison">{children}</span>
);

const optLabel = (options: OptionItem[] | null | undefined, v: string | null | undefined) =>
  options?.find((o) => o.value === v)?.label ?? '';

// IF 안으로 들어가는 노드에서 가점/감점 값을 제거(적합/부적합·미설정은 유지)
function stripPointsScore(node: FormulaNode): FormulaNode {
...
function FormulaAddZone({
  onDropLeaf,
  getDropLeaf,
  onClose,
  closable = true,
}: {
  onDropLeaf: (leaf: FormulaNode) => void;
  getDropLeaf?: (() => FormulaNode | null) | null;
  onClose?: () => void;
  closable?: boolean;
}) {
...
const COMPARE_OP_TEXT: Record<string, string> = { '>=': '이상', '>': '초과', '=': '', '<=': '이하', '<': '미만' };
...
function NaturalGroupDropStrip({
  getDropLeaf,
  onDropLeaf,
}: {
  getDropLeaf?: (() => FormulaNode | null) | null;
...
// 재귀 수식 렌더러 props — FormulaGroup/FormulaLeaf도 노드 타입만 좁혀 공유한다.
export interface ScreeningFormulaProps {
  node: FormulaNode; // leaf | group 노드(위 모델)
  root?: boolean; // 최상위 여부 — 체크박스 노출(그룹핑 대상 선택)
  checked?: boolean;
  onCheckChange?: (checked: boolean) => void; // root 전용
  onChange?: (nextNode: FormulaNode) => void;
  onDelete?: () => void; // X 버튼
  getDropLeaf?: () => FormulaNode | null; // 드래그 중인 카드의 leaf(없으면 null). '조건 추가' 드롭 존이 사용
  ...
  variant?: FormulaVariant; // 'formula'(수식 표기) | 'natural'(자연어 문장 표기 — 같은 노드 모델·기능, 8466:6316)
  inIf?: boolean; // IF 함수 서브트리 안 여부 — 조건 값은 적합/부적합만 허용(2026-07-16)
  className?: string;
}
...
function FormulaGroup({ node, root, checked, onCheckChange, onChange, onDelete, getDropLeaf, catalog, variant = 'formula', inIf = false, className }: Omit<ScreeningFormulaProps, 'node'> & { node: FormulaGroupNode }) {
  ...
  const removeAddZone = (id: string) => setAddZoneIds((prev) => prev.filter((x) => x !== id));
  const patch = (p: Partial<FormulaGroupNode>) => onChange?.({ ...node, ...p });
  ...
  const ifScoreInput = (key: 'trueScore' | 'falseScore', fallback?: string) => (
    <GroupScoreInput
  ...
  const replaceChild = (idx: number, next: FormulaNode) => {
    const children = node.children.slice();
    children[idx] = next;
    patch({ children });
  };

  const removeChild = (idx: number) => patch({ children: node.children.filter((_, i) => i !== idx) });
...
const SCORE_TEXT: Record<string, string> = { fit: '적합', unfit: '부적합' };
...
function leafSummary(node: FormulaLeafNode, catalog?: FormulaCatalog | null) {
  const condMeta =
...
function nodeSummary(node: FormulaNode, catalog?: FormulaCatalog | null): string | null {
  if (node.kind === 'group') {
...
function ChipTrigger({ children, open }: { children?: ReactNode; open?: boolean }) {
  return (
    <span
...
function FormulaLeaf({ node, root, checked, onCheckChange, onChange, onDelete, catalog, variant = 'formula', inIf = false, className }: Omit<ScreeningFormulaProps, 'node'> & { node: FormulaLeafNode }) {
  const patch = (p: Partial<FormulaLeafNode>) => onChange?.({ ...node, ...p });
  const startRef = useRef<HTMLSpanElement | null>(null); // 이 수식의 체인 시작점(조건 종류 칩)
  ...
  const saveBundleCondition = (v: ScreeningConditionValue, close: () => void) => {
    if (v.items) {
      onChange?.({ ...node, valueTab: v.tab ?? null, value: null, items: v.items });
      close();
      setTimeout(() => document.querySelector<HTMLElement>(`[data-indiv-for="${node.id}"] input`)?.focus(), 0);
    } else {
      onChange?.({
        ...node,
        valueTab: v.tab ?? null,
            ...
            {(close) => (
              <ScoreSettingMenu
                ...
                onSave={({ type, points }) => {
                  ...
                  patch({
                    scoreType: type as FormulaLeafNode['scoreType'],
