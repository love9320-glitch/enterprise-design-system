...
import { FORMULA_GROUP_FUNCTIONS, hasPointsScore } from '../components/formulaFunctions';
import type {
  ConditionTab,
  FormulaCatalog,
  FormulaLeafNode,
...
// 조건 카드 데이터 — { id, name, conditionTabs?, conditionOptionsByTab?, conditionOptions?, condition?, score? }
export interface ScreeningCard {
  ...
  condition?: { tab?: string | null; option?: string | null; items?: string[] } | null;
  score?: { type: string; points?: string } | IndividualSetting | null;
}
// onChange 스냅샷 — 카드에서 id/name/condition/score만 추린 것
type ScreeningCardSnapshot = Pick<ScreeningCard, 'id' | 'name' | 'condition' | 'score'>;

// 수식 영역(존) — 존마다 독립된 수식 트리
...
// '추가' rich menu 항목/섹션 — [ [ { title, items: [...] } ] ] (컬럼 배열 > 섹션 배열 > 항목)
...
// ref 핸들 — getCards() · addCard(card) · getFormulas()
...
interface ScreeningBuilderTemplateProps extends Omit<ComponentPropsWithoutRef<'div'>, 'onChange'> {
  defaultCards?: ScreeningCard[]; // 초기 카드 목록(mount 1회) — { id, name, conditionTabs?, conditionOptionsByTab?, conditionOptions?, condition?, score? }
  onChange?: (cards: ScreeningCardSnapshot[]) => void; // 카드 순서/값/삭제 변경 시 스냅샷
  onFormulaChange?: (formulas: ScreeningZone[]) => void; // 수식 트리 변경 시 스냅샷
  cardWidth?: number | string; // 카드 너비: 숫자(px) | CSS 길이 | 'fill'
  formulaArea?: boolean; // 수식 영역 표시 여부 — false면 카드 목록만
  onAddCard?: () => void; // 카드 영역 '추가' 버튼 클릭 — cardMenu 미지정 시에만 사용(없으면 빈 카드 추가)
  // cardMenu — '추가' 버튼의 rich menu 데이터(컬럼 배열 > 섹션 배열 > 항목).
  //   항목 클릭 시 조건 카드가 맨 위에 등록되고,
  //   팝오버는 계속 열려 있어 연속 추가 가능(배경 클릭으로만 닫힘)
  cardMenu?: ScreeningCardMenuSection[][];
  boxHeight?: number; // 빌더 박스 고정 높이(px) — 내용이 넘치면 내부 스크롤(ScrollArea)
}
...
const flatOptions = (card: ScreeningCard) =>
  card.conditionTabs?.length
    ? Object.values(card.conditionOptionsByTab ?? {}).flat()
    : (card.conditionOptions ?? []);

export const ScreeningBuilderTemplate = forwardRef<
  ScreeningBuilderTemplateHandle,
  ScreeningBuilderTemplateProps
>(function ScreeningBuilderTemplate(
  {
    defaultCards = [],
    onChange,
    onFormulaChange,
  ...
  const [cards, setCards] = useState<ScreeningCard[]>(() =>
    defaultCards.map((c) => ({ condition: null, score: null, ...c })),
  );

  const snapshot = (list: ScreeningCard[]): ScreeningCardSnapshot[] =>
    list.map(({ id, name, condition, score }) => ({ id, name, condition, score }));
  const emit = (next: ScreeningCard[]) => {
    setCards(next);
    onChange?.(snapshot(next));
  };

  // 빌더 모드 — '수식'(드래그·그룹핑 빌더) / '자연어'(추후 자연어 생성 방식 — 현재 빈 영역)
  const [builderMode, setBuilderMode] = useState<'formula' | 'natural'>('formula');
  // 수식 영역(존) 목록 — 존마다 아코디언 리스트 하나 + 독립된 수식 트리. '수식 영역 추가'로 늘린다.
  const [zones, setZones] = useState<ScreeningZone[]>(() => [
    { id: newId('zone'), name: '수식 영역 1', formulas: [] },
  ]);
  const [checkedIds, setCheckedIds] = useState<string[]>([]); // 그룹핑 대상 체크(모든 존 공통 — id는 유일)
  const [groupFn, setGroupFn] = useState<string | null>(null);
  const emitZones = (next: ScreeningZone[]) => {
    setZones(next);
    onFormulaChange?.(next.map((z) => ({ id: z.id, name: z.name, formulas: z.formulas })));
  };
  const patchZone = (zoneId: string, fn: (formulas: FormulaNode[]) => FormulaNode[]) =>
  ...
  const patchCard = (id: string, patch: Partial<ScreeningCard>) =>
    emit(cards.map((c) => (c.id === id ? { ...c, ...patch } : c)));
  const removeCard = (id: string) => emit(cards.filter((c) => c.id !== id));
  ...
  const addCardFromMenu = (item: ScreeningCardMenuItem) =>
    emit([
      {
        id: newId('card'),
        name: item.name,
  ...
  const [dropHoverId, setDropHoverId] = useState<string | null>(null); // 드래그 중 하이라이트된 존 id
  ...
  const formulaCatalog: FormulaCatalog = {
  ...
  const baseLeaf = (card: ScreeningCard): FormulaLeafNode => ({
    id: newId('formula'),
    kind: 'leaf',
    criteria: card.id,
  ...
  const signedPoints = (type: string | null, points?: string) => {
    const raw = (points ?? '').replace(/^-/, '');
    return type === 'minus' && raw ? `-${raw}` : type === 'plus' ? raw : '';
  };
  const cardToLeaf = (card: ScreeningCard): FormulaLeafNode => {
    const type = card.score?.type ?? null;
    return {
      ...baseLeaf(card),
      value: card.condition?.option ?? null,
      valueTab: card.condition?.tab ?? null,
      points: signedPoints(type, (card.score as { points?: string } | null)?.points),
      scoreType: type as FormulaLeafNode['scoreType'], // 'fit'|'unfit'이면 점수 란에 적합/부적합 텍스트 표시
    };
  };
  ...
  const cardToNode = (card: ScreeningCard): FormulaNode => {
    ...
    if (items?.length) {
      return {
        ...
        individual: card.score?.type === 'individual' ? (card.score as IndividualSetting) : null,
      };
    }
    return cardToLeaf(card);
  };
  // 카드는 드롭해도 목록에 남는다 — 같은 조건을 여러 번 드롭해 수식을 계속 추가할 수 있다(2026-07-16 지시)
  const dropCardToZone = (zoneId: string) => {
  ...
  const zoneCheckedIds = (zone: ScreeningZone) =>
  ...
  const ifAllowedInZone = (zone: ScreeningZone) => {
  ...
  const capAllowedInZone = (zone: ScreeningZone) => {
    const picked = zone.formulas.filter((f) => checkedIds.includes(f.id));
  ...
  const fnAllowedInZone = (fn: string, zone: ScreeningZone) => {
  ...
  const groupCheckedInZone = (zoneId: string) => {
  ...
  const ungroupCheckedInZone = (zoneId: string) => {
    const zone = zones.find((z) => z.id === zoneId);
    if (!zone) return;
    const pickedIds = zoneCheckedIds(zone);
    emitZones(
