...
import type { TableColumn, TableRowKey } from '../components/Table';
import type { IndividualItemSetting, IndividualSetting, OptionItem } from '../components/formulaFunctions';

// 항목 draft — 가점/감점(points) 모드와 적합/부적합(fitness) 모드 분리 보관용
type PointsItem = Extract<IndividualItemSetting, { type: 'plus' | 'minus' }>;
type FitItem = Extract<IndividualItemSetting, { type: 'fit' | 'unfit' }>;
...
const MODE_TABS = (
  fitnessOnly: boolean,
): { value: 'points' | 'fitness'; label: string; disabled?: boolean }[] => [
  { value: 'points', label: '가점/감점', disabled: fitnessOnly },
  { value: 'fitness', label: '적합/부적합' },
];

interface ScreeningIndividualSettingModalProps {
  items?: OptionItem[]; // [{ value, label }] — 지정된 항목(카드의 선택 항목, 옵션 순서)
  value?: IndividualSetting | null; // 저장된 값(위 페이로드) | null — 열 때 draft 시드
  fitnessOnly?: boolean; // 적합/부적합 전용 — IF 함수 안 조건은 가점/감점 불가(2026-07-16)
  onClose: () => void; // Modal 계약상 필수(딤·X·Esc 닫기) // 닫기(취소·X·딤·Esc)
  onConfirm?: (payload: IndividualSetting) => void; // 확인 시에만
}
...
export function ScreeningIndividualSettingModal({
  ...
  onClose,
  onConfirm,
}: ScreeningIndividualSettingModalProps) {
  const [mode, setMode] = useState<IndividualSetting['mode']>(
    fitnessOnly ? 'fitness' : (value?.mode ?? 'points'),
  );
  // 모드별 draft를 분리 보관 — 탭을 오가도 입력이 유지되고, 확인 시 활성 모드만 저장된다
  const seed = <T extends IndividualItemSetting>(m: IndividualSetting['mode'], defaults: T) => {
    const out: Record<string, T> = {};
    items.forEach((it) => {
      const saved = value?.mode === m ? value?.items?.[it.value] : null;
      out[it.value] = (saved as T | null | undefined) ?? defaults;
    });
    return out;
  };
  const [pointsDraft, setPointsDraft] = useState<Record<string, PointsItem>>(() =>
    seed('points', { type: 'plus', points: '' }),
  );
  const [fitDraft, setFitDraft] = useState<Record<string, FitItem>>(() => seed('fitness', { type: 'fit' }));
  // 일괄 적용 — 체크한 행에만 적용(2026-07-16 지시). 모드가 바뀌면 셀렉트 옵션이 달라지므로 초기화
  const [bulkType, setBulkType] = useState('');
  const [bulkPoints, setBulkPoints] = useState('');
  const [checkedIds, setCheckedIds] = useState<TableRowKey[]>([]); // 테이블 체크박스 — 일괄 적용 대상(행 id=항목 값 문자열)
  const changeMode = (m: IndividualSetting['mode']) => {
    setMode(m);
    setBulkType('');
    setBulkPoints('');
  };
  ...
  const applyBulk = () => {
    if (!bulkReady) return;
    const set = isPoints
      ? { type: bulkType as PointsItem['type'], points: bulkPoints.trim() }
      : { type: bulkType as FitItem['type'] };
    const patchChecked = <T extends IndividualItemSetting>(d: Record<string, T>) =>
      ({
        ...d,
  ...
  const patchRow = (val: string, patch: Partial<PointsItem> | Partial<FitItem>) =>
    isPoints
      ? setPointsDraft((d) => ({ ...d, [val]: { ...d[val], ...patch } as PointsItem }))
      : setFitDraft((d) => ({ ...d, [val]: { ...d[val], ...patch } as FitItem }));
  ...
  const complete = isPoints
    ...
    : true;
  const payload = (): IndividualSetting => ({
    type: 'individual',
    mode,
    items: Object.fromEntries(
      items.map((it): [string, IndividualItemSetting] =>
        isPoints
    ...
    { key: 'label', label: '항목' },
    {
      ...
      render: (row) => (
        <Select
          ...
          value={(isPoints ? pointsDraft : fitDraft)[String(row.id)]?.type ?? ''}
          onChange={(e) => patchRow(String(row.id), { type: e.target.value as 'plus' | 'minus' | 'fit' | 'unfit' })}
          menuWidth={100}
        />
      ),
    },
            ...
            render: (row) => (
              <span className="flex items-center gap-spacing-4">
                <Input
                  size="32"
                  width={70}
                  value={pointsDraft[String(row.id)]?.points ?? ''}
                  onChange={(e) => patchRow(String(row.id), { points: digitsOnly(e.target.value) })}
