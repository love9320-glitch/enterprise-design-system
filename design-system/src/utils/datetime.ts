...
const toTimeStr = (time?: string | Date | null) => {
  if (!time) return '';
...
export function formatDateTime(date?: Date | null, time?: string | Date | null): string {
  if (!date) return '';
...
export function formatDateTimeRange(
  ...
  { emptyStartLabel = '시작일 없음', emptyEndLabel = '마감일 없음' }: { emptyStartLabel?: string; emptyEndLabel?: string } = {},
): string {
