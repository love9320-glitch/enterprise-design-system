...
import { JobPositionTemplateB } from '../components/JobPositionTemplateB';
...
const USAGE = `import { JobPositionTemplateB } from '../components/JobPositionTemplateB';

<JobPositionTemplateB
...
const USAGE_PROPS = [
  ...
  { name: '라벨들', type: 'string', default: "'채용 분야' 등", desc: '카피 커스텀(orderLabel/jobLabel/manageLabel/emptyMessage/emptyValueMessage) — B타입은 섹션 타이틀 없음(테이블 헤더가 조건 구성 표시)' },
];


export function JobPositionTemplateBPage() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-spacing-7 py-spacing-10 text-left">
      <h2 className="mb-spacing-3 text-20 font-semibold text-font-icon-5">Job Position Template B</h2>
      <p className="mb-spacing-9 text-14 text-font-icon-4">
        채용 분야 설정 템플릿 B타입 — A타입과 다른 UX 실험 버전입니다. 상하 레이아웃(조건 카드 가로 배치)이고, 카드에는 드래그·사용 스위치·타이틀 없이 기준 셀렉트만 있으며,
      ...
      <p className="mb-spacing-6 text-12 text-font-icon-4">
        ...
        고릅니다 — 칩 플레이스홀더에 조건명이 들어갑니다("지역을 선택하세요"). 기준을 더 고르면
