...
import { SideNavigationTemplate } from '@gusun/design-system';
import { Input } from '@gusun/design-system';
import { Button } from '@gusun/design-system';
import { Table } from '@gusun/design-system';
...
const USAGE = `import { SideNavigationTemplate } from '@gusun/design-system';
...
<SideNavigationTemplate
  ...
  adding={adding}
  onAddSubmit={(name: any) => addCategory(name)}  // 입력 후 Enter → 생성
  onAddCancel={() => setAdding(false)}
  onRenameMenu={(id: any, name: any) => renameCategory(id, name)}
  onDeleteMenu={(id: any) => deleteCategory(id)}
>
...
function Demo({ navWidth = 180, line = true, showAdd = true, showArrow = true, addPosition = 'top', overflow = 'ellipsis', height, extraMenus = 0 }: any) {
  const [rows, setRows] = useState(INITIAL_ROWS);
  const [menu, setMenu] = useState('all');
  const [draft, setDraft] = useState('');
  const [userCats, setUserCats] = useState<any[]>([]); // 사용자가 더한 카테고리 — [{ id, name }] (id 고정: 삭제해도 안 밀림)
  const [adding, setAdding] = useState(false); // '카테고리 추가' 클릭 → 이름 입력 행 표시

  // 메뉴 id → 필터할 카테고리명(추가 더미 메뉴는 라벨 자체를 카테고리로 사용)
  const catNameOf = (id: any) =>
    (CATEGORY_BY_MENU as any)[id] ??
    (id.startsWith('extra-')
      ? `카테고리 ${Number(id.slice(6)) + 6}`
      : userCats.find((c: any) => c.id === id)?.name ?? null);
  const visible = menu === 'all' ? rows : rows.filter((r: any) => r.category === catNameOf(menu));
  const countOf = (key: any) => (key === 'all' ? rows.length : rows.filter((r: any) => r.category === catNameOf(key)).length);
  const menus = [
    ...
    ...userCats.map((c: any) => ({ id: c.id, label: c.name, count: countOf(c.id) })),
  ];

  // '카테고리 추가'(+) 클릭 → 이름 입력 행(adding), Enter로 확정 시 생성 + 바로 선택
  const addCategory = (name: any) => {
    setAdding(false);
    const id = `user-${Date.now()}`;
    setUserCats((prev: any) => [...prev, { id, name }]);
    setMenu(id);
  };
  // 이름 수정 — 그 카테고리로 추가된 코드 행들의 카테고리명도 함께 바꿔 필터 연결 유지
  const renameCategory = (id: any, name: any) => {
    const old = userCats.find((c: any) => c.id === id)?.name;
    setUserCats((prev: any) => prev.map((c: any) => (c.id === id ? { ...c, name } : c)));
    if (old) setRows((prev: any) => prev.map((r: any) => (r.category === old ? { ...r, category: name } : r)));
  };
  // 삭제 — 그 카테고리의 코드 행도 함께 정리, 보고 있던 카테고리면 '전체'로 이동
  const deleteCategory = (id: any) => {
    const old = userCats.find((c: any) => c.id === id)?.name;
    setUserCats((prev: any) => prev.filter((c: any) => c.id !== id));
    if (old) setRows((prev: any) => prev.filter((r: any) => r.category !== old));
  ...
  const addCode = () => {
    ...
    setRows((prev: any) => [...prev, { id: Date.now(), category, name }]);
    setDraft('');
  };
...
function Playground() {
  const [opts, setOpts] = useState<Record<string, any>>({ line: true, showAdd: true, showArrow: true, manyMenus: false });
      ...
      <div className="mb-spacing-7 rounded-round-4 border border-base-gray-100 px-spacing-7 py-spacing-6">
        <div className="flex min-h-[32px] flex-wrap items-center gap-x-spacing-9 gap-y-spacing-5">
          <label className="inline-flex items-center gap-spacing-4 text-14 text-font-icon-5">
            내비 너비
            <Select options={WIDTH_OPTIONS as any} value={navWidth as any} onChange={(e: any) => setNavWidth(Number(e.target.value))} width={140} />
          </label>
          ...
          <label className="inline-flex items-center gap-spacing-4 text-14 text-font-icon-5">
            추가 행 위치
            <Select options={ADD_POSITION_OPTIONS} value={addPosition} onChange={(e: any) => setAddPosition(e.target.value)} width={170} />
          </label>
          <label className="inline-flex items-center gap-spacing-4 text-14 text-font-icon-5">
            긴 명칭(overflow)
            <Select options={OVERFLOW_OPTIONS} value={overflow} onChange={(e: any) => setOverflow(e.target.value)} width={190} />
          </label>
          <label className="inline-flex items-center gap-spacing-4 text-14 text-font-icon-5">
            높이 제한(height)
            <Select options={HEIGHT_OPTIONS as any} value={height as any} onChange={(e: any) => setHeight(['none', 'fill'].includes(e.target.value) ? e.target.value : Number(e.target.value))} width={230} />
          </label>
