...
const USAGE = `import { MultiStepFormTemplate } from '@gusun/design-system';
...
function SenderStep({ method, onMethodChange }: { method: MethodKey; onMethodChange: (m: MethodKey) => void }) {
...
function TargetsStep() {
  ...
  const [rows, setRows] = useState<(typeof TARGET_ROWS)[number][]>([]);
...
function ReviewStep({ method }: { method: MethodKey }) {
...
export function MultiStepFormTemplatePage() {
  ...
  const [method, setMethod] = useState<MethodKey>('both');
