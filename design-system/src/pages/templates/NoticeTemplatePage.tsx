...
function NumField({ label, value, onChange, disabled }: any) {
  return (
...
const USAGE = `import { NoticeWritingTemplate } from '@gusun/design-system/editor';
...
function Playground() {
  ...
  const num = (setter: any) => (e: any) => setter(Math.max(0, Number(e.target.value) || 0));
      ...
      <div className="flex flex-col gap-spacing-6 rounded-round-4 border border-base-gray-100 p-spacing-7">
        ...
        <div className="flex flex-wrap items-center gap-x-spacing-9 gap-y-spacing-5">
          <Checkbox
            ...
            checked={showTabs}
            onChange={(e: any) => setShowTabs(e.target.checked)}
          />
          <Checkbox
            label="showAttach (첨부파일 버튼)"
            checked={showAttach}
            onChange={(e: any) => setShowAttach(e.target.checked)}
          />
          <Checkbox
            label="mergeFields (머지필드 툴바)"
            checked={useMerge}
            onChange={(e: any) => setUseMerge(e.target.checked)}
          />
          <Checkbox
            label="editorReadOnly (읽기 전용)"
            checked={readOnly}
            onChange={(e: any) => setReadOnly(e.target.checked)}
          />
          <Checkbox
            label="editorMaxHeight 제한 (끄면 무제한)"
            checked={capMax}
            onChange={(e: any) => setCapMax(e.target.checked)}
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-x-spacing-9 gap-y-spacing-5">
          <div className="flex items-center gap-spacing-5">
            ...
            <Select
              value={tabVariant}
              onChange={(e: any) => setTabVariant(e.target.value)}
              options={[
                { value: 'hug', label: 'hug' },
