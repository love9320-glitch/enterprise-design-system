...
const USAGE = `import { TableTemplate } from '@gusun/design-system';
...
<TableTemplate
  ...
  dragHandleColKey="type" rowDragLabel={(row: any) => row.title}
/>`;
...
const STATUS_OPTIONS = STATUSES.map((s: any) => ({ value: s, label: s }));
const APPLY_OPTIONS = [1, 2, 3, 4, 5].map((n: any) => ({ value: `${n}개`, label: `${n}개` }));
const EVAL_OPTIONS = [1, 2, 3].map((n: any) => ({ value: `${n}개`, label: `${n}개` }));
...
const COLUMNS = [
  { key: 'type',   label: '구분',     width: 80,  render: () => <Tag color="blue">태그</Tag> },
  {
    ...
    render: (row: any) => <Tag color={(STATUS_TAG as any)[row.status]}>{row.status}</Tag>,
  },
  {
    key: 'title', label: '공고명',
    headerMenu: { sortable: true }, // 헤더 우측 ⋮ 메뉴 — 오름/내림차순 정렬
    render: (row: any) => (
      <Button variant="underline" truncate onClick={(e: any) => e.stopPropagation()}>{row.title}</Button>
    ),
  },
  { key: 'period', label: '접수 기간', width: 280, headerMenu: { sortable: true } },
  {
    key: 'apply', label: '지원서', width: 110,
    render: (row: any) => <Select variant="text" size="24" menuWidth={80} options={APPLY_OPTIONS} defaultValue={row.apply} />,
  },
  {
    key: 'eval', label: '평가', width: 110,
    render: (row: any) => <Select variant="text" size="24" menuWidth={80} options={EVAL_OPTIONS} defaultValue={row.eval} />,
  },
];
...
function DemoTemplate({ withActions = true, actionSet = 'default', ...templateProps }) {
  ...
  const actions = !withActions
    ? null
    : ({ selectedIds, clearSelection }: any) => (
        <>
          ...
          {selectable && (
            <Button
              ...
              onClick={() => {
                ...
                setRows((prev: any) => prev.filter((r: any) => !removing.has(r.id)));
                clearSelection();
              }}
            >
  ...
  return (
    <TableTemplate
      ...
      rowDragLabel={(row: any) => row.title}
      {...rest}
    />
  );
}
...
function Playground() {
  ...
  const playColumns =
    opts.draggable && opts.dragInCell
      ? COLUMNS.map((c: any) => (c.key === 'type' ? { ...c, width: 102 } : c))
      : COLUMNS;
          ...
          {opts.pagination ? (
            <>
              ...
              <label className="inline-flex items-center gap-spacing-4 text-14 text-font-icon-5">
                번호 개수
                <Select
                  options={MAX_BUTTONS_OPTIONS as any}
                  value={maxButtons as any}
                  onChange={(e: any) => setMaxButtons(Number(e.target.value))}
                  width={100}
                />
              </label>
            </>
            ...
            <label className="inline-flex items-center gap-spacing-4 text-14 text-font-icon-5">
              표 높이
              <Select
                options={HEIGHT_OPTIONS as any}
                value={maxHeight as any}
                onChange={(e: any) => setMaxHeight(Number(e.target.value))}
                width={120}
              />
            </label>
          )}
...
function Block({ title, desc, first = false, children }: any) {
  return (
    <>
