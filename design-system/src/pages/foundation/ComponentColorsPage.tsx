...
} from '@gusun/design-system/tokens';
...
const HEX_TO_BASE = (() => {
  const map = {};
  const add = (hex: any, name: any) => {
    if (typeof hex === 'string' && !(hex.toLowerCase() in map)) (map as any)[hex.toLowerCase()] = name;
  };
...
const baseNameOf = (hex: any) => (HEX_TO_BASE as any)[String(hex).toLowerCase()];
...
function flatten(obj: any, parentKey = ''): { key: string; hex: any }[] {
  const out = [];
...
function Swatch({ prefix, name, hex }: any) {
  ...
  return (
    <section className="py-spacing-10 text-left">
      ...
      <div className="divide-y divide-gray-200">
        {COMPONENTS.map(({ title, prefix, tokens }: any) => {
