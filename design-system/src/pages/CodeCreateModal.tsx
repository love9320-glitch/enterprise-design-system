...
export function CodeCreateModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const rows = ((cats as any)[current] ?? []).map((name: any, i: number) => ({ id: `${current}-${i}`, no: i + 1, name }));
  const columns = [
  ...
  const addCode = () => {
    ...
    setCats((prev: any) => ({ ...prev, [current]: [...((prev as any)[current] ?? []), name] }));
    setDraft('');
  };
  // 이름 입력 행에서 Enter — 같은 이름이 있으면 만들지 않고 그 카테고리를 선택만 한다
  const addCategory = (name: any) => {
    setAdding(false);
    setCats((prev: any) => (prev[name] ? prev : { ...prev, [name]: [] }));
    setCurrent(name);
  };
  // 이름 수정 — 순서를 유지한 채 키만 교체. 다른 카테고리와 이름이 겹치면 무시(변경 안 함)
  const renameCategory = (id: any, name: any) => {
    if (name === id || (cats as any)[name]) return;
    setCats((prev: any) => Object.fromEntries(Object.entries(prev).map(([k, v]) => [k === id ? name : k, v])));
    if (current === id) setCurrent(name);
  };
  const deleteCategory = (id: any) => {
    const rest = Object.keys(cats).filter((k: any) => k !== id);
    setCats((prev: any) => {
      const next = { ...prev };
      delete next[id];
      return next;
