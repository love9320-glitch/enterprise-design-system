...
const PURPOSE = [
  ...
  {
    no: '02',
    title: '코드 ↔ Figma 완전 동기화',
    desc: 'Code Connect로 코드와 Figma 컴포넌트를 1:1 매핑. 개발환경 컴포넌트를 Figma에서 100% 그대로 활용해 똑같이 그린다. 속성명까지 일치(예: Tag color).',
  },
  {
    no: '03',
    title: '규칙대로 UI 생성',
  ...
  },
  {
    no: '04',
    title: '규칙의 통제·진화 루프',
    desc: '규칙서를 공통/코드/Figma로 구조화하고, 사용 원장으로 빈도·마찰을 기록 → 검증 → 보완. 시스템이 자기 규칙을 보여주고 다듬는다.',
  },
];
...
export function HomePage() {
  return (
    <section className="mx-auto max-w-5xl px-spacing-7 py-spacing-10 text-left">
      ...
      <p className="mb-spacing-9 max-w-3xl text-16 leading-[1.7] text-font-icon-4">
        코드와 Figma가 100% 동기화된 단일 디자인 시스템을 만들어, 어디서든 같은 컴포넌트로
      ...
      <div className="mb-spacing-9">
        ...
        <p className="mb-spacing-7 max-w-3xl text-14 leading-[1.7] text-font-icon-4">
          ...
          일관되게 UI를 생산·확장할 수 있는 기반을 만드는 것”이 목적이었습니다.
        </p>
        <div className="grid grid-cols-1 gap-spacing-6 sm:grid-cols-2">
          {PURPOSE.map((p) => (
            ...
            >
              ...
              <p className="text-14 leading-[1.6] text-font-icon-4">{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
