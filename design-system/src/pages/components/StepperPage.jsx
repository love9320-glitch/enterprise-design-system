...
const USAGE = `import { Stepper } from '../../components/Stepper';
...
<Stepper
  ...
  onStepClick={(value) => setStep(value)} // 지정 시 클릭 내비게이션(+default 스텝 hover)
/>`;
...
export function StepperPage() {
  ...
  const items = DEMO_ITEMS.map((it) => (showDesc ? it : { ...it, description: undefined }));
    ...
    <section className="py-spacing-10 text-left">
      ...
      <div className="rounded-round-4 border border-base-gray-100 py-spacing-8">
        <Stepper
          items={items}
          value={step}
          onStepClick={clickable ? (v) => setStep(v) : undefined}
        />
      </div>
