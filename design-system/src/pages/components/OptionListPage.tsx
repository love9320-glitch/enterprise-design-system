...
import { SegmentedTabs } from '@gusun/design-system';
import { SelectGroup } from '@gusun/design-system';
...
const USAGE = `import { PopoverMenu } from '@gusun/design-system';
import { ListGroup } from '@gusun/design-system';
import { List } from '@gusun/design-system';
...
<ListGroup maxVisible={6}>
  {items.map((it: any) => <List key={it.id} title={it.label} />)}
</ListGroup>
<ListGroup empty emptyMessage="검색 결과가 없습니다" />

// PopoverMenu — topArea로 상단 영역 선택('none'|'search'|'input'). 목록은 항상 ListGroup(빈 상태 내부 처리)
const [q, setQ] = useState('');
<PopoverMenu topArea="search" searchValue={q} onSearchChange={(e: any) => setQ(e.target.value)}>
  <ListGroup>{filtered.map((it: any) => <List key={it.id} title={it.label} />)}</ListGroup>
</PopoverMenu>
...
<PopoverMenu topArea="input" inputValue={url} onInputChange={(e: any) => setUrl(e.target.value)}
...
function SearchablePopover() {
  const [q, setQ] = useState('');
  const filtered = SAMPLE.filter((s: any) => s.includes(q.trim()));
  return (
    <PopoverMenu topArea="search" searchValue={q} onSearchChange={(e: any) => setQ(e.target.value)}>
      <ListGroup>
        {filtered.map((s: any) => (
...
function Playground() {
  ...
  const [checks, setChecks] = useState<Record<string, any>>({ r1: true, r2: false, r3: false });
  const [switches, setSwitches] = useState<Record<string, any>>({ r1: true, r2: false, r3: false });
        ...
        <div className="w-[304px] overflow-hidden rounded-round-4 border border-list-popover-outline">
          {PLAYGROUND_ROWS.map((r: any) => (
            <List
              ...
              checked={checks[r.id]}
              onCheckChange={(e: any) => setChecks((s) => ({ ...s, [r.id]: e.target.checked }))}
              showSwitch={opts.showSwitch}
              switchChecked={switches[r.id]}
              onSwitchChange={(e: any) => setSwitches((s) => ({ ...s, [r.id]: e.target.checked }))}
...
function FooterDemo({ variant, label }: any) {
  const rows = SAMPLE.slice(0, 6);
  const [checked, setChecked] = useState<Record<string, any>>({});
  const selectedCount = rows.filter((r: any) => checked[r]).length;
  const allSelected = selectedCount === rows.length;

  const toggleRow = (r: any, val: any) => setChecked((c) => ({ ...c, [r]: val }));
  const selectAll = (val: any) =>
    setChecked(val ? Object.fromEntries(rows.map((r: any) => [r, true])) : {});
  ...
  const footerProps = ({
    ...
    checkbox: {
      ...
      onFooterCheckChange: (e: any) => selectAll(e.target.checked),
    },
        ...
        <ListGroup maxVisible={6}>
          {rows.map((r: any) => (
            <List
              ...
              checkbox
              checked={!!checked[r]}
              onCheckChange={(e: any) => toggleRow(r, e.target.checked)}
            />
          ))}
        </ListGroup>
      </PopoverMenu>
    ...
    <div className="flex flex-wrap items-start gap-spacing-9">
      ...
      <div>
        ...
        <PopoverMenu
          topArea="input"
          inputValue={img}
          onInputChange={(e: any) => setImg(e.target.value)}
...
function RadioListPopover() {
  const [v, setV] = useState<any>('plus');
  const [scores, setScores] = useState<Record<string, any>>({ plus: '0', minus: '0' });
  const scoreInput = (key: any) => (
    <span className="flex shrink-0 items-center gap-spacing-5" onClick={(e: any) => e.stopPropagation()}>
      <Input
        width={56}
        value={scores[key]}
        disabled={v !== key}
        onChange={(e: any) => setScores((p) => ({ ...p, [key]: e.target.value }))}
...
function TabRadioListPopover() {
  ...
  const [v, setV] = useState<any>('severe');
  const rows = [
                ...
                <PopoverMenuSection key={section.title} title={section.title} grow={si === column.length - 1}>
                  {section.items.map((label: any) => (
                    <List
                      ...
                      onButtonClick={() => setAdded((prev: any) => (prev.includes(label) ? prev : [...prev, label]))}
                    />
                  ))}
                </PopoverMenuSection>
              ))}
...
function TabSelectListPopover() {
  ...
  const [vals, setVals] = useState<Record<string, any>>({ lang: '', cat: '', level: [] }); // level=다중 선택(배열)
  const selectsDisabled = tab === 'none'; // 미보유 탭이면 셀렉트 비활성
  const changeTab = (t: any) => {
    setTab(t);
    if (t === 'none') setVals({ lang: '', cat: '', level: [] }); // 비활성으로 바뀌면 선택값 리셋
  };
  const setVal = (key: any) => (e: any) => setVals((p) => ({ ...p, [key]: e.target.value }));
  const LANG = [
    { value: 'en', label: '영어' },
    { value: 'jp', label: '일본어' },
    { value: 'cn', label: '중국어' },
