...
const USAGE = `import { Table } from '@gusun/design-system';
...
<Table columns={columns} rows={rows} rowKey="id" draggableRows onRowsReorder={(r: any) => setRows(r)} />
...
<Table columns={columns} rows={rows} rowKey="id" draggableRows dragHandleColKey="orderNo"
  rowDragLabel={(row: any) => row.title} onRowsReorder={(r: any) => setRows(r)} />`;
...
const USAGE_PROPS = [
  ...
  { name: 'maxHeight', type: "number | 'fill'", default: '—', desc: "본문 세로 스크롤 + 헤더 고정 — 숫자=고정 상한(px) / 'fill'=부모 flex 높이가 상한(모달 규칙 18). 빈 상태 + fill은 소비자가 늘려준 높이(flex-1)를 채워 엠티 문구가 중앙 정렬" },
...
const COLUMNS = [
  { key: 'type',   label: '구분',     width: 120, filter: TYPE_FILTER, render: (row: any) => <Tag color="blue">{row.type}</Tag> },
...
const H_SCROLL_COLUMNS = COLUMNS.map((c: any) =>
  c.key === 'apply'
    ? { ...c, render: (row: any) => <Button variant="underline" size="32">{row.apply}</Button> }
    : c,
);
