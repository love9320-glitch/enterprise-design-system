...
import { SideNavigation, SideNavigationButton } from '@gusun/design-system';
...
const USAGE = `import { SideNavigation, SideNavigationButton } from '@gusun/design-system';
...
function Playground({ width, line, showAdd = true }: any) {
  const [menus, setMenus] = useState<any[]>(MENUS); // 인스턴스별 자기 상태 — 수정·삭제·추가가 실제 반영
  const [active, setActive] = useState('a');
  const [editingId, setEditingId] = useState<any>(null); // 이름 수정 중인 메뉴 id
  const [draft, setDraft] = useState('');
  const [seq, setSeq] = useState(1);

  const addMenu = () => {
    setMenus((prev: any) => [...prev, { id: `new-${seq}`, label: `new menu ${seq}`, icon: FolderOpen }]);
    setSeq((n) => n + 1);
  };
  const confirmRename = (id: any) => {
    const name = draft.trim();
    if (!name) return;
    setMenus((prev: any) => prev.map((m: any) => (m.id === id ? { ...m, label: name } : m)));
    setEditingId(null);
  };
  const removeMenu = (id: any) => {
    const rest = menus.filter((m: any) => m.id !== id);
    setMenus(rest);
            ...
            actions={
              <>
                ...
                <Button
                  ...
                  aria-label={`${m.label} 삭제`}
                  tooltip="삭제"
                  onClick={() => setMenus((prev: any) => prev.filter((x: any) => x.id !== m.id))}
                />
              </>
            }
          >
