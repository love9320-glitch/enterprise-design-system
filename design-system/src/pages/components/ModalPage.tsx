...
import { Modal, AlertModal, ConfirmModal, FormModal } from '@gusun/design-system';
...
const MODAL_USAGE = `import { Modal } from '@gusun/design-system';
...
const ALERT_USAGE = `import { AlertModal } from '@gusun/design-system';
...
const CONFIRM_USAGE = `import { ConfirmModal } from '@gusun/design-system';
...
function TableInModalDemo({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    <section className="py-spacing-10 text-left">
      ...
      {SIZES.map((s: any) => (
        <Modal
          ...
          title={`size = ${s} (${(SIZE_LABEL as any)[s]})`}
          size={s}
          onConfirm={() => setSizeOpen(null)}
        >
          ModalBox 너비는 {(SIZE_LABEL as any)[s]}이고, 내부 요소는 width 100%로 채웁니다.
        </Modal>
      ))}
      ...
      >
        ...
        <Input value={name} onChange={(e: any) => setName(e.target.value)} placeholder="이름을 입력하세요" />
      </FormModal>
