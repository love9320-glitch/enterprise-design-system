...
import { ConditionOrderSlot, ConditionSlotCard } from '@gusun/design-system';
...
const USAGE = `import { ConditionOrderSlot } from '@gusun/design-system';
import { Select } from '@gusun/design-system';
...
<ConditionOrderSlot
  ...
  onOrderChange={(ids: any) => console.log('배치 순서', ids)}
...
function Playground() {
  ...
  const [applied, setApplied] = useState<any>(null); // 최근 적용 순서(사용 중 id 순)
    ...
    <div className="space-y-spacing-7">
      <div className="flex items-center gap-spacing-6">
        ...
        <Button
          ...
          size="24"
          leftIcon={Plus}
          onClick={() => setItems((prev: any) => [...prev, { id: `c${prev.length + 1}-${prev.length}`, body: cardBody() }])}
        >
          조건 카드 추가
        </Button>
      </div>
      <ConditionOrderSlot
        ...
        items={items}
        onEnabledChange={(ids: any) => setApplied(ids)}
