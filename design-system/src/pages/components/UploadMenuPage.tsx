...
import { FileUploadMenu } from '@gusun/design-system';
import { ImageUploadMenu } from '@gusun/design-system';
import { FileUploadButton } from '@gusun/design-system';
import { ImageUploadButton } from '@gusun/design-system';
import { UsageExample } from '../../components/UsageExample';
import { Divider } from '@gusun/design-system';

const USAGE = `import { FileUploadButton } from '@gusun/design-system';
import { ImageUploadButton } from '@gusun/design-system';
...
<FileUploadButton
  ...
  onAdd={(list) => setFiles((f) => [...f, ...[...list].map((x: any) => ({ name: x.name, size: Math.round(x.size / 1e6) }))])}
  onDelete={(_, i) => setFiles((f) => f.filter((_: any, idx: any) => idx !== i))}
/>
...
<ImageUploadButton
  ...
  onSelect={(file: any) => setImage(URL.createObjectURL(file))}
...
export function UploadMenuPage() {
  ...
  const addFiles = (list: any) =>
    setFiles((prev: any) => [
      ...prev,
      ...[...list].map((x: any) => ({ name: x.name, size: Math.max(1, Math.round(x.size / 1e6)) })),
    ].slice(0, 5));
  const delFile = (_: any, i: any) => setFiles((prev: any) => prev.filter((_: any, idx: any) => idx !== i));
      ...
      <div className="flex flex-wrap items-start gap-spacing-9">
        <div>
          ...
          <FileUploadMenu
            ...
            onAdd={(list) =>
              setTplFiles([...list].slice(0, 1).map((f: any) => ({ name: f.name, size: Math.round(f.size / 1e6) })))
            }
