...
const USAGE = `import { SegmentedTabs } from '@gusun/design-system';
...
function Demo({ items, ...rest }: any) {
  ...
  return (
    <div className="space-y-spacing-5">
      <SegmentedTabs items={items} value={v} onChange={(v: any) => setV(v)} {...rest} />
      <p className="text-12 text-font-icon-4">
        선택: <span className="text-font-icon-5">{items.find((it: any) => it.value === v)?.label}</span>
      </p>
    </div>
  );
}
