...
import { Chip } from '@gusun/design-system';
...
function ChipDemo() {
  ...
  return (
    <div className="flex flex-wrap items-center gap-spacing-5">
      {chips.map((c: any) => (
        <Chip key={c} onRemove={() => setChips((prev: any) => prev.filter((x: any) => x !== c))}>
          {c}
        </Chip>
      ))}
      {chips.length === 0 && (
...
const CHIP_USAGE = `import { Chip } from '@gusun/design-system';
...
const CHIP_PROPS = [
  ...
  { name: 'color', type: "'gray' | 'red' | 'blue' | 'black' | 'green' | 'violet' | 'pink' | 'orange'", default: "'gray'", desc: '색상 종류 (hover/pressed는 chip-* 토큰 경유) — green/violet/pink/orange는 수식 함수 계열 대응' },
  ...
  return (
    <section className="py-spacing-10 text-left">
      ...
      <p className="mb-spacing-9 text-14 text-font-icon-4">
        ...
        <code className="text-font-icon-5">color</code>(gray/red/blue/black + 함수 계열 대응 green/violet/pink/orange). hover/pressed 상태는 chip 토큰 경유.
      ...
      <div className="mb-spacing-7 flex flex-wrap items-center gap-spacing-5">
        {['gray', 'red', 'blue', 'black', 'green', 'violet', 'pink', 'orange'].map((c: any) => (
