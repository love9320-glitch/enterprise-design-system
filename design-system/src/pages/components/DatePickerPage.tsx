...
import { DatePicker } from '@gusun/design-system';
import { CalendarDayButton } from '@gusun/design-system';
import { TwoDepthList } from '@gusun/design-system';
import { DateField } from '@gusun/design-system';
import { TimeField } from '@gusun/design-system';
import { Checkbox } from '@gusun/design-system';
import { UsageExample } from '../../components/UsageExample';
import { formatDateTimeRange } from '@gusun/design-system';
import { Divider } from '@gusun/design-system';

const USAGE = `import { DateField } from '@gusun/design-system';
import { DatePicker } from '@gusun/design-system';

// DateField — 트리거 필드(클릭 시 DatePicker 팝오버). 값 표기는 utils/datetime 규칙으로 통일.
<DateField value={date} onChange={(v: any) => setDate(v)} />                       // 단일
<DateField mode="range" value={range} onChange={(v: any) => setRange(v)} />        // 범위
<DateField mode="range" showTime value={range} onChange={(v: any) => setRange(v)}  // 범위+시간
...
<DateField variant="text" value={date} onChange={(v: any) => setDate(v)} />
<DateField variant="text" size="20" mode="range" showTime value={range} onChange={(v: any) => setRange(v)} />

// 달력 아이콘 빼기 — box·text 공통(날짜 텍스트만)
<DateField showIcon={false} value={date} onChange={(v: any) => setDate(v)} />
<DateField variant="text" showIcon={false} value={date} onChange={(v: any) => setDate(v)} />

// DatePicker — 패널 단독(트리거에 직접 붙이려면 Popover로 감싸거나 DateField 사용)
// 단일 날짜 — value=Date | null
const [date, setDate] = useState<any>(null);
<DatePicker value={date} onChange={(v: any) => setDate(v)} />

// 범위 선택 — value={ start, end }
const [range, setRange] = useState<any>({ start: null, end: null });
<DatePicker mode="range" value={range} onChange={(v: any) => setRange(v)} />

// 지나간 날짜 선택 불가 (오늘은 가능) — minDate/maxDate/disabledDate도 지원
<DatePicker value={date} onChange={(v: any) => setDate(v)} disablePast />
...
<TwoDepthList separator="." inputValue="2025.07"  // 좌/우 분리 인풋 + '.' 구분자(아래 정렬)
  ...
  leftOptions={years}  leftValue={y}  onLeftChange={(v: any) => setY(v)}
...
function DateFieldDemo() {
  const [date, setDate] = useState<any>(null);
  const [range, setRange] = useState<any>({ start: null, end: null });
  const [rangeT, setRangeT] = useState<any>({ start: null, end: null });
  const [s, setS] = useState('00:00');
  const [e, setE] = useState('23:59');
  const [dateT, setDateT] = useState<any>(null); // 단일 + 시간
  const [st, setSt] = useState('09:00');
  const [pastRange, setPastRange] = useState<any>({ start: null, end: null });
  const [errDate, setErrDate] = useState<any>(null); // 값 없을 때만 에러 → 선택 시 툴팁 사라짐
  return (
    <div className="flex flex-col gap-spacing-8">
      <div className="flex flex-wrap items-start gap-spacing-9">
        <div>
          ...
          <DateField value={date} onChange={(v: any) => setDate(v)} />
        </div>
        <div>
          <p className="mb-spacing-4 text-12 text-font-icon-3">범위 (range)</p>
          <DateField mode="range" value={range} onChange={(v: any) => setRange(v)} />
        </div>
        <div>
        ...
        <div>
          ...
          <DateField mode="range" disablePast value={pastRange} onChange={(v: any) => setPastRange(v)} />
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-spacing-9">
        ...
        <div>
          ...
          <DateField variant="text" size="20" value={date} onChange={(v: any) => setDate(v)} />
        </div>
        <div>
...
function TimeFieldDemo() {
  ...
  const [errTime, setErrTime] = useState<any>(null); // 값 없을 때만 에러 → 선택 시 사라짐
  return (
    <div className="flex flex-wrap items-start gap-spacing-9">
      <div>
        <p className="mb-spacing-4 text-12 text-font-icon-3">기본 (비어있음)</p>
        <TimeField value={t1} onChange={(v: any) => setT1(v)} />
      ...
      <div>
        ...
        <TimeField value={t2} onChange={(v: any) => setT2(v)} />
      </div>
      <div>
        <p className="mb-spacing-4 text-12 text-font-icon-3">disabled</p>
        <TimeField disabled />
      ...
      <div>
        <p className="mb-spacing-4 text-12 text-font-icon-3">error (선택 시 사라짐)</p>
        <TimeField value={errTime} onChange={(v: any) => setErrTime(v)} error={!errTime} errorMessage="필수 선택사항입니다." />
      </div>
    </div>
  );
}
...
function DatePickerPlayground() {
  ...
  const toggle = (k: any) => setOpts((o) => ({ ...o, [k]: !o[k] }));
...
function YearMonthDemo({ showInput = true }: any) {
  ...
  return (
    <TwoDepthList
      ...
      leftOptions={YEARS}
      leftValue={y}
      onLeftChange={(v: any) => setY(v)}
      rightOptions={MONTHS}
      rightValue={m}
      onRightChange={(v: any) => setM(v)}
    />
  );
}

function TimeListDemo({ showInput = true }: any) {
  ...
  return (
    <TwoDepthList
      ...
      leftOptions={HOURS}
      leftValue={h}
      onLeftChange={(v: any) => setH(v)}
      rightOptions={MINUTES}
      rightValue={min}
      onRightChange={(v: any) => setMin(v)}
    />
  );
}
    ...
    <section className="py-spacing-10 text-left">
      ...
      <div>
        ...
        <div className="flex flex-wrap items-start gap-spacing-8">
          {DAY_STATES.map((d: any) => (
            <div key={d.state} className="flex flex-col items-center gap-spacing-4">
              <CalendarDayButton state={d.state as any}>{d.text}</CalendarDayButton>
