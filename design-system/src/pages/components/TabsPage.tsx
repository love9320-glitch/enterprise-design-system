...
import { Tabs } from '@gusun/design-system';
...
const USAGE = `import { Tabs } from '@gusun/design-system';
...
<Tabs items={items} value={tab} onChange={(v: any) => setTab(v)} />
        ...
        <ListGroup>
          {ALL.map((t: any) => (
            <List key={t.value} title={t.label} showSwitch
              ...
              onSwitchChange={(e: any) => toggle(t.value, e.target.checked)} />
          ))}
        </ListGroup>
      </PopoverMenu>
    </Popover>
...
function TabsControlDemo() {
  ...
  const [disabledSet, setDisabledSet] = useState<Set<any>>(() => new Set(['etc']));
  const [tab, setTab] = useState('profile');

  const items = ALL.map((t: any) => ({ ...t, disabled: disabledSet.has(t.value) }));

  const toggleEnabled = (value: any, enabled: any) => {
    setDisabledSet((prev: any) => {
    ...
    if (!enabled && value === tab) {
      const firstEnabled = ALL.find((t: any) => t.value !== value && !disabledSet.has(t.value));
            ...
            <ListGroup>
              {ALL.map((t: any) => (
                <List
                  ...
                  onSwitchChange={(e: any) => toggleEnabled(t.value, e.target.checked)}
                />
              ))}
            </ListGroup>
          </PopoverMenu>
...
function Playground() {
  ...
  const items = PLAYGROUND_BASE.map((t: any) => ({
    value: t.value,
    label: t.label,
  ...
  const toggleEnabled = (value: any, enabled: any) => {
    ...
    // 보고 있는 탭을 비활성화하면 선택을 첫 활성 탭으로 옮긴다
    if (!enabled && value === tab) {
      const firstEnabled = PLAYGROUND_BASE.find((t: any) => t.value !== value && !disabledSet.has(t.value));
      if (firstEnabled) setTab(firstEnabled.value);
    }
  };
  ...
  const label = PLAYGROUND_BASE.find((t: any) => t.value === tab)?.label;
...
function InteractiveTabs() {
  const [tab, setTab] = useState('profile');
  const label = ITEMS.find((i: any) => i.value === tab)?.label;
  return (
    <div>
      <Tabs items={ITEMS} value={tab} onChange={(v: any) => setTab(v)} />
      <div className="px-spacing-5 py-spacing-7 text-14 text-font-icon-4">
  ...
  return (
    <section className="py-spacing-10 text-left">
      ...
      <Block title="아이콘 · 태그" desc="items에 icon·tag/tagText를 주면 라벨 좌측 아이콘, 우측 태그가 붙습니다. tagType(blue/red/gray)으로 태그 색을 고릅니다. disabled 탭은 회색·클릭 차단.">
        <Tabs items={ICON_ITEMS as any} defaultValue="profile" />
      </Block>
