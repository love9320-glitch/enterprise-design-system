...
// ── 타입 A — 여백 그리드형 ─────────────────────────────────────────────
const USAGE_A = `import { FormTemplateA } from '../components/FormTemplateA';
...
// ── 타입 B — 테이블형 ─────────────────────────────────────────────────
const USAGE_B = `import { FormTemplateB } from '../components/FormTemplateB';
...
const titleCell = (key, span) => ({
  key,
  span,
  label: '공고명',
  control: <Input variant="transparent" width="100%" placeholder="공고명을 입력하세요" />,
});
const typeCell = (key, span) => ({
  key,
  span,
  label: '채용구분',
...
const periodCell = (key, span, { noTrailing = false } = {}) => ({
  key,
  span,
  label: '채용기간',
  control: <DateField variant="text" mode="range" showTime disablePast />, // 지난 날짜 선택 불가(2026-07-28 통일)
...
const buttonCell = (key, span, icon, text) => ({
  key,
  span,
  flush: true,
  control: (
    ...
    <div className="flex flex-col gap-spacing-7">
      ...
      <div className="rounded-round-5 border border-base-gray-100 bg-white p-spacing-8">
        <FormTemplateB
          ...
          shadow={shadow}
          round={round}
          cellPaddingTop={padTop}
          cellPaddingBottom={padBottom}
