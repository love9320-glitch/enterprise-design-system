...
import { ConfirmModal } from '@gusun/design-system';
import { MultiStepFormTemplate } from '@gusun/design-system';
...
import { Field } from '@gusun/design-system';
import { Input } from '@gusun/design-system';
import { Select } from '@gusun/design-system';
import { DateField } from '@gusun/design-system';
import { TextArea } from '@gusun/design-system';
import { Button } from '@gusun/design-system';
import { Tag } from '@gusun/design-system';
import { Divider } from '@gusun/design-system';
import { Popover } from '@gusun/design-system';
import { useHoverTooltip } from '@gusun/design-system';
import { FileUploadButton } from '@gusun/design-system';
import { PopoverMenu } from '@gusun/design-system';
import { List } from '@gusun/design-system';
import { ListGroup } from '@gusun/design-system';

// ── 공통 타입 ────────────────────────────────────────────────────────
type MethodKey = 'both' | 'email' | 'sms';
type TagColor = ComponentProps<typeof Tag>['color'];

// Step 1 폼 값 — 페이지가 소유(다음 단계 게이트 검증 대상)
...
type SetFieldFn = <K extends keyof BulkSendForm>(key: K) => (value: BulkSendForm[K] | ((prev: BulkSendForm[K]) => BulkSendForm[K])) => void;
...
const METHOD_LABEL: Record<MethodKey, string> = { both: '이메일 + 문자', email: '이메일', sms: '문자' };
...
function EmailAuthTag({ approved }: { approved: boolean }) {
...
function MappingCodeButton({ onInsert }: { onInsert?: (code: string) => void }) {
  return (
    <Popover
...
function SenderStep({ method, onMethodChange, form, setField, showErrors }: {
  ...
  const emailWriteCell: FormTemplateBCell = { key: 'emailBody', span: hasSms ? 8 : 12, paddingTop: '12', paddingBottom: '20',
    control: (
  ...
  const smsWriteCell: FormTemplateBCell = { key: 'smsBody', span: hasEmail ? 4 : 12, paddingTop: '12', paddingBottom: '20',
    control: (
    ...
    <FormTemplateB
      ...
      cells={[
        ...
        { key: 'method', label: '안내 방법', span: 4,
          control: <Select variant="text" width="fill" menuWidth="trigger" options={METHOD_OPTIONS} value={method} onChange={(e) => onMethodChange(e.target.value as MethodKey)} /> },
        { key: 'schedule', label: '예약 발송', span: 4,
          control: <DateField variant="text" width="fill" showIcon={false} showTime disablePast placeholder="발송 날짜와 시간을 선택하세요" value={form.schedule} onChange={(v) => setField('schedule')(v instanceof Date ? v : null)} error={showErrors && !form.schedule} /> },
...
const TARGET_STATUS_TAG: Record<string, TagColor> = { '발송 가능': 'blue', '정보 누락': 'red' };
...
const TARGET_COLUMNS: TableTemplateColumn[] = [
  ...
  { key: 'status', label: '상태', width: 90,
    render: (row: any) => <Tag color={TARGET_STATUS_TAG[String(row.status)]}>{String(row.status)}</Tag> },
];

type TargetRow = (typeof TARGET_ROWS)[number];

function TargetsStep({ rows, onUpload }: { rows: TargetRow[]; onUpload: () => void }) {
...
function ReviewStep({ method, targetCount }: { method: MethodKey; targetCount: number }) {
  const hasEmail = method !== 'sms';
  const hasSms = method !== 'email';
  ...
  const emailCell: FormTemplateBCell = { key: 'email', span: hasSms ? 8 : 12, paddingTop: '20', paddingBottom: '20',
    control: (
  ...
  const smsCell: FormTemplateBCell = { key: 'sms', span: hasEmail ? 4 : 12, paddingTop: '20', paddingBottom: '20',
    control: (
      <div className="flex w-full flex-col gap-spacing-7 self-start">
...
export function BulkSendPage() {
  ...
  const [method, setMethod] = useState<MethodKey>('both'); // 안내 방법 — 작성·검토 채널 구성이 따라감(정책 ②)
  const [targetRows, setTargetRows] = useState<TargetRow[]>([]); // 업로드 전 빈 상태(정책 ①) — 페이지가 소유해 스텝 무관 유지
  ...
  const [form, setForm] = useState<BulkSendForm>({ client: null, reason: '', schedule: null, emailSubject: '', emailBody: '', smsBody: '' });
  const [showErrors, setShowErrors] = useState(false); // 첫 '다음 단계' 시도 후부터 에러 표시
  const setField: SetFieldFn = (key) => (value) =>
