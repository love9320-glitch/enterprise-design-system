...
import { Modal } from '@gusun/design-system';
import { JobPositionTemplate } from '@gusun/design-system';
import { Button } from '@gusun/design-system';
import { Tag } from '@gusun/design-system';
import { TableTemplate } from '@gusun/design-system';
import { NoticeWritingTemplate } from '@gusun/design-system/editor';
...
function BackgroundModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    ...
    >
      <TableTemplate
        ...
        onSelectChange={(ids: any) => setSelectedIds(ids)}
        defaultPageSize={5}
...
const SELECT_COLUMNS = [
  ...
  { key: 'status', label: '상태', width: 100, render: (row: any) => <Tag color={(STATUS_TAG as any)[row.status]}>{row.status}</Tag> },
...
function SelectionModal({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    },
    {
      ...
      control: (
        <Select
          ...
          value={methods}
          onChange={(e: any) => setMethods(e.target.value)}
    ...
    >
      <div className="flex flex-col gap-spacing-9">
        ...
        <NoticeWritingTemplate
          key={template || 'empty'}
          mergeFields={NOTICE_MERGE_FIELDS}
          enabledChannels={methods as any}
          defaultBodies={template ? (NOTICE_TEMPLATE_BODIES as any)[template] : undefined}
          editorMinHeight={360}
        />
      </div>
    </Modal>
...
function MessageTemplateModal({ open, onClose }: { open: boolean; onClose: () => void }) {
...
function EvalSettingsModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [selectedIds, setSelectedIds] = useState<any[]>([]);
  const close = () => {
    setSelectedIds([]);
    onClose();
  ...
  const columns = [
    {
      ...
      cellSpan: (row: any) => row.titleSpan,
      render: (row: any) => <span className="whitespace-normal break-words text-14">{row.title}</span>,
    },
    {
      key: 'field',
      label: '채용 분야',
...
function ItemAddModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const [items, setItems] = useState(() => [
    ...ITEM_BASE_NAMES.map((name: any, i: number) => ({ id: `base-${i}`, name, base: true })),
  ...
  const addItem = () => {
    ...
    if (!name) return;
    setItems((prev: any) => [...prev, { id: `item-${Date.now()}`, name, base: false }]);
    setDraft('');
  };
  const removeItem = (id: any) => setItems((prev: any) => prev.filter((it: any) => it.id !== id));
  const startEdit = (row: any) => {
    setEditingId(row.id);
    setEditText(row.name);
  };
  const commitEdit = () => {
    setItems((prev: any) => prev.map((it: any) => (it.id === editingId ? { ...it, name: editText.trim() || it.name } : it)));
    setEditingId(null);
  };

  const rows = items.map((it: any, i: number) => ({ ...it, order: i + 1 }));
  const columns = [
    {
      key: 'order',
      label: '순서',
      width: 60,
      render: (row: any) => <span className="text-font-icon-3">{row.order}</span>,
    },
    {
      key: 'name',
      label: '명칭',
...
function JobPositionModal({ open, onClose, onRegisterCode }: any) {
