...
export function applyColumnFilters(
  rows: TableRowData[],
  columns: { key: string; filter?: unknown }[],
...
export function applySort(rows: TableRowData[], sort?: TableSort | null) {
...
export function iconCellWidth(
  count = 1,
  { buttonSize = 24, gap = parseInt(spacing['spacing-5'], 10) }: { buttonSize?: number; gap?: number } = {},
) {
