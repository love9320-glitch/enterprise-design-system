...
// ...props는 바깥 필드 컨테이너(div)로, 실제 <textarea> 속성은 textareaProps로 전달한다.
interface TextAreaProps extends Omit<ComponentPropsWithoutRef<'div'>, 'onChange'> {
  value?: string;
  defaultValue?: string;
  onChange?: (e: ChangeEvent<HTMLTextAreaElement>) => void;
  placeholder?: string;
  rows?: number;            // 기본 높이(줄 수). autoGrow=false면 고정 높이(초과 시 스크롤), true면 최소 높이
  autoGrow?: boolean;       // 입력량에 따라 높이 자동 증가
  maxRows?: number;         // autoGrow일 때 최대 행(초과 시 스크롤). 미지정 + autoGrow = 무한 증가
  maxLength?: number;       // 최대 글자수(입력 제한 + 카운터 분모)
  showCount?: boolean;      // 글자수 카운터 표시(Figma countTxt) — maxLength 있으면 'N/max', 없으면 'N'
  disabled?: boolean;
  readOnly?: boolean;
  error?: boolean;
  errorMessage?: string;
