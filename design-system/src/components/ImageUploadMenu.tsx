...
// onSelect는 DOM select 이벤트 핸들러와 이름이 겹쳐 Omit 후 파일 콜백으로 재선언
export interface ImageUploadMenuProps extends Omit<PopoverMenuProps, 'onSelect'> {
  /** 미리보기 이미지 URL (없으면 empty) */
  image?: string;
  /** 안내 문구(여러 줄은 \n) */
  guide?: string;
  /** <input accept> 실제 필터 (예: '.jpeg,.png,.webp'). ⚠️ guide 표시와 직접 일치시킬 것 */
  accept?: string;
  /** (File) => void — '파일 찾기'로 선택 */
