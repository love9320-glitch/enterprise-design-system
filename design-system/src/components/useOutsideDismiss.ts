...
export function useOutsideDismiss({ open, refs, onDismiss, guard }: any) {
  useEffect(() => {
    if (!open) return;
    const onDown = (e) => {
      ...
      if (active instanceof HTMLElement && refs.some((r) => r.current && r.current.contains(active))) active.blur();
      onDismiss(e);
    };
