...
// onInputApply 실패 반환형 — 해당 파트 인풋 아래에 그 문구로 툴팁
...
interface ColumnProps {
  options: TwoDepthOption[];
  value?: TwoDepthOption['value'];
  onChange?: (value: TwoDepthOption['value']) => void;
  maxVisible?: number;
}

// 한 컬럼 — 옵션 목록 + 선택 행 자동 스크롤(열릴 때 선택값이 보이도록).
function Column({ options, value, onChange, maxVisible }: ColumnProps) {
...
const splitParts = (text: string, separator: string): [string, string] => {
...
interface TwoDepthListProps extends ComponentPropsWithoutRef<'div'> {
  ...
  inputValue?: string;
  separator?: string;       // 좌/우 파트 구분자 표시·합성 기준 — year_month '.' / time ':'
  showInput?: boolean;      // 상단 입력 영역 표시 여부(false면 좌/우 컬럼만 — 트리거가 따로 입력을 가질 때)
  editable?: boolean;       // 상단 input 직접 입력 허용(false면 readOnly로 값만 표시)
  onInputChange?: (e: ChangeEvent<HTMLInputElement>) => void; // (e) => void — 입력 변경(raw 이벤트)
  ...
  errorMessage?: string; // 파트 지정이 없을 때의 기본 툴팁 메시지
  // 파트 입력 허용 문자(문자 1개씩 test) — 기본은 숫자만(구분자는 UI가 표시하므로 입력 불필요).
  allowedChars?: RegExp | null;
  inputPlaceholder?: string; // 'YYYY.MM'·'HH:MM'처럼 구분자 포함 문자열 — 구분자로 쪼개 좌/우 placeholder로 사용
  inputProps?: ComponentPropsWithoutRef<'input'>;
  // 좌측 컬럼
  leftOptions?: TwoDepthOption[];  // [{ value, label, disabled? }]
  leftValue?: TwoDepthOption['value'];
  onLeftChange?: (value: TwoDepthOption['value']) => void;
  // 우측 컬럼
  rightOptions?: TwoDepthOption[]; // [{ value, label, disabled? }]
  rightValue?: TwoDepthOption['value'];
  onRightChange?: (value: TwoDepthOption['value']) => void;
  maxVisible?: number;             // 각 컬럼 표시 행 수(초과 시 내부 스크롤)
  width?: number | string;
}
...
export function TwoDepthList({
  ...
  const [parts, setParts] = useState<[string, string]>(() => splitParts(inputValue, separator));
  const [error, setError] = useState<{ side: 'left' | 'right'; message: string } | null>(null); // { side:'left'|'right', message } | null
  ...
  const lastSideRef = useRef<'left' | 'right'>('left'); // 파트 미지정 실패(null) 시 툴팁을 붙일 마지막 편집 인풋
  ...
  const compose = (l: string, r: string) => `${l}${separator}${r}`;

  const handlePartChange = (side: 'left' | 'right') => (e: ChangeEvent<HTMLInputElement>) => {
    // 허용 문자만 남긴다(기본 숫자만 — 구분자는 UI가 항상 표시).
    const v = allowedChars
      ? Array.from(e.target.value).filter((c) => allowedChars.test(c)).join('')
      : e.target.value;
    const next: [string, string] = side === 'left' ? [v, parts[1]] : [parts[0], v];
  ...
  const handleFocus = (side: 'left' | 'right') => (e: FocusEvent<HTMLInputElement>) => {
    lastSideRef.current = side;
    setFocused(true);
  ...
  const handleBlur = (e: FocusEvent<HTMLInputElement>) => {
    ...
    if (norm && !(typeof norm === 'object' && norm.error)) {
      ...
      const side = (typeof norm === 'object' && norm !== null && norm.error) || lastSideRef.current;
      ...
      setError({ side, message: (typeof norm === 'object' && norm !== null && norm.message) || errorMessage });
    }
  };
  ...
  const partInput = (side: 'left' | 'right', value: string, placeholder: string) => (
