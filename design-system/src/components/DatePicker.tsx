...
// 선택값(단일=Date|null, 범위=DateRange)
export type DatePickerValue = Date | DateRange | null;
// 연/월·시/분 2depth 목록 옵션
...
const fmtYearMonth = (d: Date) => `${d.getFullYear()}.${pad2(d.getMonth() + 1)}`;
...
function buildWeeks(viewDate: Date, weekStartsOn: number) {
...
function buildWindow(monthDate: Date, weekStartsOn: number) {
...
interface CalendarScrollerProps {
  anchorMonth: Date;
  anchorKey: number;
  weekStartsOn: number;
...
function CalendarScroller({ anchorMonth, anchorKey, weekStartsOn, onVisibleMonthChange, renderDay }: CalendarScrollerProps) {
  const vpRef = useRef<HTMLDivElement | null>(null);
  const setVp = useCallback((el: HTMLDivElement | null) => {
    vpRef.current = el;
  }, []);
  const [weeks, setWeeks] = useState<Date[]>(() => buildWindow(anchorMonth, weekStartsOn));
  const lastKeyRef = useRef(anchorKey); // 마지막으로 헤더에 보고/반영한 달
  const adjustRef = useRef<'anchor' | number>('anchor');   // 다음 레이아웃에서 scrollTop 보정: 'anchor' | px(number) | 0
...
interface DualSelectFieldProps {
  icon?: LucideIcon;
  display?: ReactNode;
  panelWidth?: number;
  placement?: string;
  disabled?: boolean;
  // ── 이하 TwoDepthList 패스스루 (TwoDepthListProps와 시그니처 일치) ──
  separator?: string;
  inputValue?: string;
  inputPlaceholder?: string;
  /** 직접 입력 적용 — 정규화 문자열 반환 또는 { error: 'left'|'right', message } */
  onInputApply?: (text: string) => string | { error: 'left' | 'right'; message: string };
  leftOptions?: DepthOption[];
  leftValue?: string;
  onLeftChange?: (value: string | number) => void;
  rightOptions?: DepthOption[];
  rightValue?: string;
  onRightChange?: (value: string | number) => void;
  maxVisible?: number;
}

function DualSelectField({ icon: Icon, display, panelWidth = 160, placement = 'auto-left', disabled = false, ...panelProps }: DualSelectFieldProps) {
...
interface TimeFieldProps {
  ...
  hourOptions: DepthOption[];
  minuteOptions: DepthOption[];
  placement?: string;
  disabled?: boolean;
  justify?: 'center' | 'between';
}

function TimeField({ label, value, onChange, hourOptions, minuteOptions, placement = 'auto-right', disabled = false, justify = 'center' }: TimeFieldProps) {
  const [hh = '00', mm = '00'] = (value || '').split(':');
  const set = (h: string | number, m: string | number) => onChange?.(`${h}:${m}`);
  return (
    <div
...
export interface DatePickerProps
  ...
  /** 오늘 이전 날짜 선택 불가(오늘은 선택 가능) */
  disablePast?: boolean;
  /** 오늘 이후 날짜 선택 불가(오늘은 선택 가능) */
  disableFuture?: boolean;
  /** 이 날짜 이전은 비활성 (Date) */
  minDate?: Date;
  /** 이 날짜 이후는 비활성 (Date) */
  maxDate?: Date;
  /** (date) => boolean — 커스텀 비활성 판정 */
  disabledDate?: (date: Date) => boolean;
  /** 표시 월(controlled) — Date */
  month?: Date;
  defaultMonth?: Date;
  onMonthChange?: (month: Date) => void;
  /** 하단 시간 영역 표시 — 범위=시작/마감 2칸 · 단일=시간 1칸(startTime 사용, 마감 없음) */
  showTime?: boolean;
  startTime?: string;
  endTime?: string;
...
export function DatePicker({
  ...
  const current = isControlled ? (value as DatePickerValue) : internalValue;
  const range = isRange ? (current as DateRange | null) ?? { start: null, end: null } : null;
  const single = !isRange ? (current as Date | null) ?? null : null;

  // 범위 모드 hover 안내 툴팁 — 호버한 셀 위에 "시작일/마감일"을 띄운다(포털, 클리핑 방지).
  const [hover, setHover] = useState<{ rect: DOMRect; label: string } | null>(null); // { rect, label } | null
