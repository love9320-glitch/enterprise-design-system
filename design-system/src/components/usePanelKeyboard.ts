...
interface UsePanelKeyboardOptions {
  complete: boolean; // 저장 가능 상태 — 마지막 요소 Tab에서 onCommit 실행 조건
  onCommit: () => void; // 저장 + 다음 진행
  onSkip?: (() => void) | null; // 미완성 이탈 — 없으면 처음으로 순환
}

export function usePanelKeyboard({ complete, onCommit, onSkip }: UsePanelKeyboardOptions) {
