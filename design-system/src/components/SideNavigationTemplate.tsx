...
export function SideNavigationTemplate({
  menus = [],              // [{ id, label, count?, icon?, showNewTag?, newTagColor?, disabled? }] — count는 0이면 표시 안 함
  selectedId: selectedProp, // controlled 선택 id
  defaultSelectedId,        // uncontrolled 초기 선택
  onSelect,                 // (id) => void
  navWidth = 180,           // 좌측 내비 너비 (Figma 180/220/260)
  line = true,              // 내비 우측 구분선
  showAdd = true,           // 추가(+) 행 — 메뉴 추가 기능 on/off
  addLabel = 'add menu',
  addPosition = 'top',      // 'top'(Figma 템플릿 기본) | 'bottom' — 추가 행 위치
  onAdd,
  overflow = 'ellipsis',    // 메뉴 라벨 긴 경우 처리 — SideNavigationButton 패스스루
