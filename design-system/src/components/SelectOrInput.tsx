...
import type { SelectPlacement } from './Select';
...
// ...props는 루트 컨테이너(div)로 전달된다.
interface SelectOrInputProps extends Omit<ComponentPropsWithoutRef<'div'>, 'onChange'> {
  value?: string;
  defaultValue?: string;
  onChange?: (e: { target: { value: string; source: SelectOrInputSource } }) => void; // (e) => void — e.target.value(문자열) · e.target.source('select'|'input'|null)
  options?: (string | OptionItem)[]; // 등록된 값 목록: 문자열 | { value, label }
  showSelect?: boolean;     // false면 Select 숨김 — 직접 입력 전용(Input이 전체 폭)
  showInput?: boolean;      // false면 Input 숨김 — 등록값 선택 전용(Select가 전체 폭)
  selectPlaceholder?: string;
  inputPlaceholder?: string;
  disabled?: boolean;
  readOnly?: boolean;
  error?: boolean;
  errorMessage?: string;    // Input의 에러 툴팁으로 표시
  formatInput?: (raw: string) => string; // (raw) => string — 직접 입력값 변환(예: 전화번호 하이픈). 입력 시마다 적용
  width?: number | string;  // 전체 너비: 숫자(px) | CSS 길이('100%' 등)
  selectWidth?: number | string; // Select 너비 — 나머지는 Input이 채움
  menuWidth?: number | string;   // Select 드롭다운 너비(미지정 시 트리거와 동일)
  placement?: SelectPlacement;
  searchable?: boolean;     // Select 검색 필터
  inputProps?: ComponentPropsWithoutRef<'input'>; // 내부 <input> 속성 통과(inputMode 등)
}
...
export function SelectOrInput({
  ...
  const deriveSource = (val: string): SelectOrInputSource => {
    if (!val) return null;
  ...
  const [lastSource, setLastSource] = useState<SelectOrInputSource>(() => deriveSource(defaultValue));
  ...
  const emit = (val: string, src: SelectOrInputSource) => {
...
interface PhoneFieldProps extends Omit<SelectOrInputProps, 'formatInput'> {
  formatInput?: ((raw: string) => string) | null; // 기본: 하이픈 자동 포맷(끄려면 null)
}
