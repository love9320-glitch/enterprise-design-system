...
interface TimeFieldProps {
  value?: string | null;                 // 'HH:MM' | null(값 없음)
...
export function TimeField({
  ...
  const applyValue = (v: string | null) => {
    ...
    >
      {() => (
        ...
        <TwoDepthList
          ...
          leftOptions={HOURS}
          leftValue={hh}
          onLeftChange={(h: string | number) => applyValue(`${h}:${mm || '00'}`)}
          rightOptions={MINUTES}
          rightValue={mm}
          onRightChange={(m: string | number) => applyValue(`${hh || '00'}:${m}`)}
          maxVisible={5}
        />
      )}
    </Popover>
