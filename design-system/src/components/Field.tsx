...
const DEFAULT_GAP: Record<'vertical' | 'horizontal', keyof typeof GAP_CLASS> = {
  vertical: 'spacing-3',
...
interface FieldProps extends ComponentPropsWithoutRef<'div'> {
  label?: ReactNode;                        // string | ReactNode — 주면 내부 Label 렌더
  htmlFor?: string;                         // 컨트롤 id와 연결(라벨 클릭 시 포커스). 컨트롤에 같은 id 부여 필요
  required?: boolean;
  disabled?: boolean;                       // 라벨 비활성 스타일(컨트롤 disabled는 컨트롤에 직접 전달)
  description?: ReactNode;                  // 보조 설명(helper) — 컨트롤 아래 회색 텍스트
  direction?: 'vertical' | 'horizontal';    // 라벨↔컨트롤 배치
