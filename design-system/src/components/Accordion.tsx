...
export function Accordion({ children, className = '', ...props }: ComponentPropsWithoutRef<'div'>) {
...
export interface AccordionItemProps extends ComponentPropsWithoutRef<'div'> {
  ...
  /** 이름 편집 버튼(SquarePen) 노출 */
  nameEditable?: boolean;
  /** (title) => void — 저장 시에만(취소/Esc는 폐기) */
  onTitleChange?: (title: string) => void;
  /** 삭제 버튼(Trash2) 노출 */
  deletable?: boolean;
  /** 삭제 버튼 비활성(예: 마지막 항목은 삭제 불가) */
  deleteDisabled?: boolean;
  /** () => void */
  onDelete?: () => void;
  /** 접힌 상태에서도 본문 마운트(폼 상태 유지 등) */
  keepMounted?: boolean;
  /** 본문 내부 스크롤 상한(px) — 미지정 시 자연 확장(페이지 스크롤) */
