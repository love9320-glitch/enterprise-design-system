...
interface SelectGroupProps extends ComponentPropsWithoutRef<'div'> {
  ...
  width?: 'hug' | 'fill';       // 'hug'(콘텐츠 폭) | 'fill'(부모 전체 폭 — 셀렉트들 균등 분할)
}
...
export function SelectGroup({
  ...
  const rendered = flattenChildren(children).map((c, i) => {
    ...
    // Select 자식의 width prop 주입용 좁힘(런타임 동작 동일 — 타입만 명시)
    const sc = c as ReactElement<{ width?: number | string }>;
    const el = c.type === Select ? cloneElement(sc, { width: sc.props.width ?? '100%' }) : c;
    return (
