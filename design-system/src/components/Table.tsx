...
// 행 선택 키(rowKey 값 또는 인덱스 폴백)
export type TableRowKey = string | number;

// 헤더 필터(filter 컬럼) 옵션 — Select options 계약과 동일(값·라벨 문자열)
...
// 헤더 메뉴(items) 항목 — sortable 자동 항목 외 임의 기능 추가용
...
function HeaderMenu({
  ...
  const menu = column.headerMenu!; // headerMenu가 있을 때만 렌더된다(호출부 가드)
...
export function Table({
  ...
  const activeFilters = filtersControlled ? (filters as Record<string, unknown>) : internalFilters;
  ...
  const activeSort = sortControlled ? (sort as TableSort | null) : internalSort;
  ...
  const [internalSel, setInternalSel] = useState<TableRowKey[]>([]);
  const selected = isControlled ? (selectedIds as TableRowKey[]) : internalSel;
  const selectedSet = new Set(selected); // O(1) 조회 — 행마다 includes 하지 않게

  const getKey = (row: TableRowData, i: number) => (row[rowKey] ?? i) as TableRowKey;
  // 선택 + 인덱스 key 폴백 + 정렬/필터가 겹치면 체크가 다른 행으로 옮겨간다 — 개발 중에만 경고
  const meta = import.meta as { env?: { DEV?: boolean } };
  if (meta.env?.DEV && selectable && rows.length > 0 && rows[0]?.[rowKey] == null) {
    console.warn(
      `[Table] selectable인데 rows에 rowKey('${rowKey}')가 없어 인덱스로 선택을 추적합니다 — 정렬/필터 시 선택이 다른 행으로 이동합니다. 고유 키 필드를 지정하세요.`,
    );
  }
  const emit = (next: TableRowKey[]) => {
  ...
  const cellLine = (isLast: boolean) => (isLast ? '' : 'border-r border-table-cell-line');
  const headLine = (isLast: boolean) => (isLast ? '' : 'border-r border-table-header-line');
  ...
  const headDivider: CSSProperties | undefined = bordered
  ...
  const headCorner = (isFirst: boolean, isLast: boolean) =>
  ...
  const headCellProps = (isFirst: boolean, isLast: boolean, width?: number) => ({
  ...
  const headCell = (c: TableColumn, isFirst: boolean, isLast: boolean) => (
    <th key={c.key} {...headCellProps(isFirst, isLast, c.width)}>
      ...
      <div className={`flex min-w-0 flex-1 items-center ${ALIGN_STYLE[c.align as keyof typeof ALIGN_STYLE] ?? ALIGN_STYLE.left}`}>
          ...
          (() => {
            ...
            return (
              <Select
                variant="text"
                size="20"
                value={(activeFilters[c.key] ?? '') as string}
        ...
        {c.headerMenu && (c.headerMenu.sortable || (c.headerMenu.items?.length ?? 0) > 0) && (
  ...
  const tableStyle: CSSProperties = { borderSpacing: 0, ...(tableMinWidth ? { minWidth: tableMinWidth } : null) };
