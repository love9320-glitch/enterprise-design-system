...
function ToolbarButton({ icon, label, active = false, disabled = false, onClick }: any) {
  return (
    <Button
      variant="ghost"
      size="24"
...
function DropTrigger({ icon, label, ariaLabel }: any) {
  if (label) {
    return (
      <Button
        variant="ghost"
...
function ColorPanel({ palette, current, onPick }: any) {
  return (
...
function UrlInputForm({ initialUrl = '', placeholder, submitLabel, onSubmit, onRemove, removeLabel = '해제' }: any) {
...
// 링크 입력 폼(URL 적용/해제).
function LinkForm({ editor, close }: any) {
  const apply = (url) => {
...
function ImageForm({ editor, close }: any) {
  const insert = (url) => {
...
function MergeFieldMenu({ editor, mergeFields, close }: any) {
...
export function EditorToolbar({ editor, toolbar, mergeFields = [] }: any) {
