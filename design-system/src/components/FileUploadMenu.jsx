...
export function FileUploadMenu({
  ...
  guide,                   // 안내 문구(여러 줄은 \n)
  maxCount = 5,
  accept,                  // <input accept> 실제 필터 (예: '.pdf,.doc,.xlsx'). ⚠️ guide 표시와 직접 일치시킬 것
  multiple = true,
  onAdd,                   // (FileList) => void — '파일 찾기'로 선택
  onDelete,                // (file, index) => void — 행 삭제
  findText = '파일 찾기',
  maxText,                 // 기본 `최대 ${maxCount}개까지 업로드할 수 있습니다`
  width = 420,
  className = '',
  ...props
}) {
