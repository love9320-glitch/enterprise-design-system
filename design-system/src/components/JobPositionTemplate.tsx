...
// 테이블 로우 — { id, items: [{ criteria, value }] }
interface JobRow {
  id: string;
  items: JobRowItem[];
}
...
interface JobPositionTemplateProps extends Omit<ComponentPropsWithoutRef<'div'>, 'onChange'> {
  ...
  defaultDisabledIds?: string[]; // 초기 미사용(스위치 off) 조건 id 목록 — 예: ['cond-4'] (id=`cond-1`..`cond-N`)
  ...
  jobLabel?: string;
  manageLabel?: string;
  tableHeight?: number | 'fill'; // 테이블 세로 — 'fill'(내용만큼 계속 확장, 넘치면 모달 바디 스크롤) | 숫자(px 고정 상한+바디 스크롤)
  emptyMessage?: string;
...
export function JobPositionTemplate({
  ...
  const setCardValue = (id: string, value: string | string[]) =>
  ...
  const labelOf = (criteria: string, value: string) =>
  ...
  const activeComplete = effOrder
    ...
    .filter((s): s is JobSelection => !!(s.criteria && (Array.isArray(s.value) ? s.value.length > 0 : s.value)));
  // 미리보기 — 다중 값 세그먼트는 라벨을 ', '로 잇는다(예: "서울 / 신입 / 프론트엔드, 백엔드")
  const valueLabel = (s: JobSelection) =>
  ...
  const expandCombos = (): JobRowItem[][] => {
    const idx = activeComplete.findIndex((s) => Array.isArray(s.value));
    if (idx === -1)
      return [activeComplete.map((s) => ({ criteria: s.criteria, value: s.value as string }))];
    return (activeComplete[idx].value as string[]).map((v) =>
      activeComplete.map((s, i) => ({ criteria: s.criteria, value: i === idx ? v : (s.value as string) })),
    );
  };
                  ...
                  <Select
                    ...
                    {...({
                      ...
                      onChange: (e: { target: { value: string | string[] } }) =>
                        setCardValue(id, e.target.value),
