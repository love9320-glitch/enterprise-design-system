...
export interface ConditionTab {
  value: string;
  label: string;
  disableOptions?: boolean; // 옵션 없이 저장 가능(미보유/비대상 등)
  multiSelect?: boolean; // 복수 조건(지정) 탭 — 다중 선택 Select
  placeholder?: string;
  searchPlaceholder?: string;
  bundleLabel?: string; // 수식 묶음 칩 표기(기본=탭 라벨)
}
...
export type IndividualItemSetting = { type: 'plus' | 'minus'; points?: string } | { type: 'fit' | 'unfit' };
export interface IndividualSetting {
  type: 'individual';
  mode: 'points' | 'fitness';
  items: Record<string, IndividualItemSetting>;
  fn?: string; // 과거 데이터 호환(함수 셀렉트 제거 전 저장분)
}
export interface FormulaLeafNode {
  id: string;
  kind: 'leaf';
  criteria?: string | null; // 조건 카드 id
  value?: string | null; // 선택 옵션 값
  valueTab?: string | null;
  points?: string; // 부호 포함 문자열('-3' 등)
  scoreType?: 'plus' | 'minus' | 'fit' | 'unfit' | null;
  display?: 'full' | 'compact';
  items?: string[]; // 묶음 leaf(복수 조건) — 지정 항목 값 배열
  individual?: IndividualSetting | null; // 항목별 개별설정
  // 드롭 시점 스냅샷(카드 삭제 대비 폴백 — 표시는 catalog 라이브 우선)
...
export interface FormulaGroupNode {
  ...
  trueScore?: string; // IF — 참일 때 점수(기본 '1')
  falseScore?: string; // IF — 거짓일 때 점수(기본 '0')
  capScore?: string; // CAPMAX/CAPMIN — 제한 점수
  compareOp?: string; // FITBYSCORE/UNFITBYSCORE — 비교 연산자(기본 '>=')
  compareScore?: string; // FITBYSCORE/UNFITBYSCORE — 비교값
}
...
export const FORMULA_FN_FAMILY: Record<string, string> = {
  AND: 'logical',
  OR: 'logical',
  IF: 'conditional',
  SUM: 'aggregate',
...
export function focusNextChainStop(currentEl: HTMLElement | null) {
  if (!currentEl) return;
  // 패널 언마운트가 끝난 다음 틱에 이동(포커스 경합 방지)
  setTimeout(() => {
    const all = Array.from(document.querySelectorAll<HTMLElement>('[data-formula-chain]'));
    const next = all[all.indexOf(currentEl) + 1];
    if (!next) return;
    const target = next.matches('[tabindex]') ? next : next.querySelector<HTMLElement>('[tabindex]');
    if (!target) return;
    target.focus();
...
export function chainTriggerKey(
  e: { defaultPrevented: boolean; key: string; shiftKey: boolean; preventDefault: () => void },
  open: boolean,
  onOpen: () => void,
) {
...
export function hasPointsScore(node: FormulaNode): boolean {
