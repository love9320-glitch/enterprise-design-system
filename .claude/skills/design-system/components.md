...
| Label | `components/Label.jsx` | children · htmlFor · size(12/13/14/15/16 — Figma `label` Property 1) · required(빨강 `*`) · disabled(font-icon-3) — 폼 라벨 텍스트(font-icon-5/Pretendard Regular). Figma는 사이즈 변형만, required/disabled/htmlFor는 코드 전용 추가 옵션(규칙 11). 단독 사용 + Field 내부 재사용. **Code Connect 연결**(Figma `label` 7942:2175) | `pages/LabelPage.jsx` |
