...
| Field | `components/Field.jsx` | label(string/node→내부 Label) · htmlFor · required · disabled · description(helper) · direction(vertical/horizontal) · labelWidth(가로 라벨폭) · labelSize · gap(간격 토큰 키) · children(컨트롤) — 라벨+컨트롤(Input/Select/DateField 등)을 묶는 폼 행. 컨트롤은 children으로 그대로 조립(규칙4). **에러는 소유하지 않음** — 컨트롤 자체 error 툴팁 사용(중복 회피). 내부에서 Label 재사용 | `pages/FieldPage.jsx` |
