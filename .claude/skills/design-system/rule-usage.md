...
|---|---|---|---|
| 1a 컬러 Base→Semantic→컴포넌트 경유 | 6 | 07-03 | 시멘틱 신설 ask-정책 정착(Label→Field helper→Divider→Chip 계보, 누락 0이면 ask 없이 확장). 07-03: 공유 토큰(cell-line) 한쪽만 바꿀 땐 분리 신설(header-line), 스케일에 없는 중간값은 base 중간 키(gray-125) 신설 후 시멘틱이 참조. 판례 원문 → rule-usage-archive.md §1a |
...
| 2a 등록 토큰만(spacing/round/border) | 10 | 07-03 | 등록 토큰만 정착 — 작성 전 토큰 확인이 임의값 재발을 예방. 07-02 없는 간격(10px)은 ask 후 중간 키 spacing-5-5 신설(컬러 ask-방침의 간격 확장 판례). 07-03 SelectGroup도 GAP_STYLE 정적 lookup 재사용. 컴포넌트 고정 치수(h-[45px] 등)는 임의 px 허용 관례. 판례 원문 → rule-usage-archive.md §2a |
...
| 4c 조합/배치 | 9 | 07-03 | 조합/배치 정착 — 모달=Modal 셸+템플릿 slot 조립이 표준(안내 작성·안내 및 발표·항목 추가 코드/Figma 양쪽, NoticeWritingTemplate=Tabs+Input+Button+Editor 조립). 판례 원문 → rule-usage-archive.md §4c |
...
| 8a 말줄임 hover 툴팁(TruncatingText) | 1 | 07-03 | 07-03 정합 갱신: TruncatingText 구현 완료·적용 중(Button·Select·List·InlineFieldTrigger, DateField는 동일 패턴 자체 구현) — 규칙 8 문구를 '목표(보류)'→'구현 완료·사용'으로 승격 |
| 8b 적용 시 위치·잘림 검증 | 0 | – | |
...
| 15 카피 띄어쓰기: 맞춤법 + 복수 허용 시 맥락 우선 | 2 | 07-03 | **2026-07-03 사용자 지시로 신설**(SKILL.md A구역). 맞춤법 준수, 둘 다 맞으면 이해하기 쉬운 쪽 — 한 단위 필드명은 붙임("발신주소"), 서술형 구는 띄움("안내 및 발표 명칭" — 붙였다가 사용자 되돌림이 규칙 신설 계기). 같은 용어는 시스템 안에서 표기 통일. 신설 직후 전수 스캔: 위반 6곳 수정(FormTemplatePage 라벨 2곳=모달과 표기 불일치, TextAreaPage '글자수'→'글자 수' 4곳=의존명사 원칙). '안내 문구/문자'는 올바른 띄어쓰기라 오탐 주의. 코드 주석은 카피가 아니라 대상 외. |
...
| 세부 규칙 | 적용 | 최근 | 마찰·애매 메모 |
|---|---|---|---|
| 11a Code Connect 기능 훼손 금지 | 9 | 07-03 | Code Connect 매핑 원칙 정착 — 셋 단위=코드 컴포넌트 1개(variant 쪼개기 불가), 미게시 오류는 재시도 먼저(전파 지연), add 직후 readback·맵 캐시/서브트리 주의, Figma placeholder prop명에 코드 API 안 맞춤(규칙 11b). 07-03 SelectGroup 10변형 매핑. 판례 원문 → rule-usage-archive.md §11a |
