...
|---|---|---|---|
| 1a 컬러 Base→Semantic→컴포넌트 경유 | 8 | 07-06 | 시멘틱 신설 ask-정책 정착(Label→Field helper→Divider→Chip 계보, 누락 0이면 ask 없이 확장). 07-03: 공유 토큰(cell-line) 한쪽만 바꿀 땐 분리 신설(header-line), 스케일에 없는 중간값은 base 중간 키(gray-125) 신설 후 시멘틱이 참조. 07-03 NewTag: Figma 변수(new tag/bg·title) 값이 전부 기존 base 매핑(blue.400/red.400/gray.500/white, 누락 0)이라 ask 없이 newTagColors.js 신설 — Chip 판례 재적용. 07-06 SideNavigation도 동일(add side navi/* → sideNavColors.js, select-bg=blue-400-50 정확 매핑). 판례 원문 → rule-usage-archive.md §1a |
...
| 2a 등록 토큰만(spacing/round/border) | 11 | 07-06 | 등록 토큰만 정착 — 작성 전 토큰 확인이 임의값 재발을 예방. 07-02 없는 간격(10px)은 ask 후 중간 키 spacing-5-5 신설(컬러 ask-방침의 간격 확장 판례). 07-03 SelectGroup도 GAP_STYLE 정적 lookup 재사용. 07-06 side nav 버튼 내부 gap이 Figma에 9px(토큰 없음) → ask 정책대로 확인, 사용자 선택=spacing-5(8px, 디자인 오차 처리) — '없는 간격=ask' 3번째 판례. 텍스트 행간도 text-14 기본(24) 대신 leading-20 토큰 조합으로 Figma(14/20) 일치. 컴포넌트 고정 치수(h-[45px] 등)는 임의 px 허용 관례. 판례 원문 → rule-usage-archive.md §2a |
...
| 3c 기존 DS 자산 우선·신규 제작 지양 | 14 | 07-02 | 기존 자산 우선 정착 — 새 요구는 새 컴포넌트 대신 기존 옵션/조립으로(Select multiple·SelectOrInput 베이스·useOutsideDismiss 훅 추출·FormTemplate=grid+Field 조립·Divider 전면 교체·머지필드 칩=chip 토큰 재사용 등). 판례 원문 → rule-usage-archive.md §3c |
...
| 8b 적용 시 위치·잘림 검증 | 1 | 07-06 | 07-06 SideNavigationButton ellipsis — 실제 마우스 hover E2E로 포털 툴팁 표시 검증(잘린 라벨 전체 노출 확인) |
| 9a ScrollArea 사용 | 3 | 07-06 | **06-30 첫 적용 + textarea 예외 정립:** TextArea의 네이티브 `<textarea>`는 ScrollArea(div용 JS 오버레이)로 못 감쌈. **시도1: `.custom-scrollbar` CSS로 네이티브 바 스타일링 → hover 불안정(Firefox 불가)으로 실패(사용자 "그대로야").** **시도2: auto-grow+ScrollArea로 감싸기 → 커서 추적(caret) 깨짐(예상).** **정답(채택): 네이티브 스크롤(커서 추적) 유지 + `.hide-native-scroll`로 기본 바만 숨기고, ScrollArea와 같은 오버레이 thumb(scroll-bar 토큰·hover·드래그, ScrollArea 세로 thumb 계산 그대로 포팅)를 textarea 스크롤에 동기화해 얹음.** 규칙 9에 이 패턴을 textarea 예외로 명시, `.custom-scrollbar`는 제거. 07-06 Table bordered+maxHeight: sticky 헤더가 알파 배경이라 스크롤 행이 비침(사용자 지적) → 헤더를 스크롤 밖 분리, 바디만 ScrollArea(maxHeight) — 동일 colgroup+table-fixed로 컬럼 정렬, 가로는 바깥 ScrollArea로 헤더·바디 동반. 남은 한계: 가로 오버플로 시 세로 thumb가 콘텐츠 우측(뷰포트 밖)에 붙음(실사용 빈도 낮아 보류). 07-06 SideNavigation 메뉴 목록도 ScrollArea로 — **부모 유동 높이엔 뷰포트에 `max-h-full`이 핵심**(maxHeight prop 없이 flex-1 루트만으론 뷰포트가 안 줄어듦 — App.jsx absolute inset-0 방식의 flex 버전 판례). 같은 날 Table bordered 분리 바디에도 동일 패턴 적용(maxHeight='fill'+minHeight — min 생략=hug, 상한=모달 가용 높이·ModalBodyMaxContext 체인). 이후 noneline도 분리 방식으로 통일(sticky 경로 삭제 — 헤더 위로 행 스크롤 잔상 제거, fill도 두 타입 지원). |
...
| 15 카피 띄어쓰기: 맞춤법 + 복수 허용 시 맥락 우선 | 3 | 07-06 | **2026-07-03 사용자 지시로 신설**(SKILL.md A구역). 맞춤법 준수, 둘 다 맞으면 이해하기 쉬운 쪽 — 한 단위 필드명은 붙임("발신주소"), 서술형 구는 띄움("안내 및 발표 명칭" — 붙였다가 사용자 되돌림이 규칙 신설 계기). 같은 용어는 시스템 안에서 표기 통일. 신설 직후 전수 스캔: 위반 6곳 수정(FormTemplatePage 라벨 2곳=모달과 표기 불일치, TextAreaPage '글자수'→'글자 수' 4곳=의존명사 원칙). '안내 문구/문자'는 올바른 띄어쓰기라 오탐 주의. 코드 주석은 카피가 아니라 대상 외. **07-06 재발(사용자 지적): Figma 원문 '채용 코드 명'을 그대로 옮겨 placeholder '채용 코드명'과 표기 불일치 발생 → '채용 코드명' 통일(2곳). 교훈: Figma에서 가져오는 텍스트도 카피 규칙 필터를 거칠 것 — 원문 그대로가 면제 사유 아님(Figma 쪽 원문도 정리 권장).** |
| 17 아이콘 전용 셀 width=iconCellWidth 계산 | 1 | 07-03 | **2026-07-03 사용자 지시로 신설**(templates/list-page.md + 체크리스트). 고스트 아이콘 버튼만 단독인 셀은 임의 폭 대신 `tableView.iconCellWidth(n,{buttonSize,gap})`=셀 좌우 패딩(spacing-5-5×2)+버튼 폭+간격(spacing-5) 합산 — 토큰 추종. 신설 시점 사용처는 '항목 추가' 모달 1곳(76px)뿐이며 이미 적용됨. render의 gap 클래스와 인자 gap 일치 필요(불일치 시 폭 어긋남) — 주의 메모. |
