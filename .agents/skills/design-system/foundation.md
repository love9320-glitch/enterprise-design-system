...
> 새 컴포넌트에 컬러가 필요하면 `fontIconColors.js` 또는 `colors/xxxColors.js`에 **시멘틱 토큰을 먼저 추가**한 뒤 `tokens/index.ts`에 export하고 사용한다. (컴포넌트 컬러 파일은 base를 `'../colors.js'`로 참조)
...
## 날짜·시간 표기 규칙 (`src/utils/datetime.ts`)
...
- [ ] `tokens/index.ts`에 export를 추가했는가
